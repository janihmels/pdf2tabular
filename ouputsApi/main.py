...
def artistxrevxhalf(df: pd.DataFrame):
    ...
    output = pd.concat([pd.DataFrame({"Release Artist" : ""},index=[0]), output], axis=0)
    return output
...
def payorXincomeXtypeXrevXhalf(df: pd.DataFrame):
    ...
    for third_party, third_party_df in third_parties:
        ...
        third_party_output = pd.concat([pd.DataFrame({"Third Party": ""}, index=[0]), third_party_output], axis=0)

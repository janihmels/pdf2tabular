...
for j in statement_period_half:
  select_table_2 += (''' sum( CASE WHEN Statement_Period_Half_9LC = "{}"
                        THEN Royalty_Payable_SB ELSE NULL END) AS `{}`,'''.format(j,j))

select_table_3 = '''sum( Royalty_Payable_SB) AS `Total`
                 FROM Master
                 GROUP BY Song_Name_9LC, Normalized_Income_Type_9LC ORDER BY `Total` DESC'''
...
column_names = ['Song Name', 'Income Type']

...
def incomecountrypayeepercentrev(database, filename):
...
#Find number of songs
  find_songs = '''SELECT Song_Name_9LC, Country_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Country_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB'''
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Song,Country,Income,Payee %,Rev"

  #Build table
  select_table_1 = '''SELECT Song_Name_9LC, Country_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,  Payee_Percent_Received_SB, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC <> "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Country_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB ORDER BY `Total` DESC'''.format(cut_off)
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()

  #Add pool revenue line at bottom
  pool_rev_1 = '''SELECT Song_Name_9LC, Country_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC = "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Country_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB'''.format(cut_off)
  mycursor.execute(pool_rev_1)
  pool_revenue_1 = mycursor.fetchall()
  final_table_1 = table_1 + pool_revenue_1

  #Create list of column names
  column_names_1 = ['Song Name', 'Country', 'Normalized Income Type', 'Normalized Income Type II', 'Payee Percent Received', 'Total Revenue']

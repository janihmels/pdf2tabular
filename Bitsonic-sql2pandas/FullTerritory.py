...
def fullterritoryxrevxhalf(database, df, filename):
...
#Find number of countries
  find_countries = '''SELECT Country_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Country_SB'''
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Block,Region,Country x Rev x {}".format(period_name)

  #Build table
  select_table_1_1 = '''SELECT Regional_Block_SB, Region_SB, Country_SB,'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}"
                   ...
                   GROUP BY Regional_Block_SB, Region_SB, Country_SB ORDER BY `Total` DESC'''.format(cut_off)
  ...
  for country_no in range(2, country_count + 2):
    ...
    ws.cell(row=country_no, column=2).alignment = Alignment(horizontal="left", vertical="center")
    ws.cell(row=country_no, column=3).alignment = Alignment(horizontal="left", vertical="center")

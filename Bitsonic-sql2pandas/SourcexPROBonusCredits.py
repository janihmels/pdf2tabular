...
def sourcexPRObonuscreditsxhalf(database, df, filename):
...
#Create sheets
  wb = Workbook()
  ws = wb.active
  ws.title = 'Source x PRO Bonus Credits x Half'
...
#Build sheet
  ...
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Normalized_Source_9LC <> "Pool Revenue" 
                           THEN Bonus_Credits_SB ELSE "" END) AS `{} {}`,'''.format(j,mySQL_column,k, j,k))
  select_table_1_3 = '''sum( CASE WHEN Normalized_Source_9LC <> "Pool Revenue" AND {} >= "{}"
                   THEN Bonus_Credits_SB ELSE "" END) AS `Total`
  ...
  for l, m in zip(year_statement_list, statement_list):
    pool_rev_1_2 += ('''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Normalized_Source_9LC = "Pool Revenue"
                      THEN Bonus_Credits_SB ELSE "" END) AS `{} {}`,'''.format(l,mySQL_column,m, l,m))
  ...
  column_names_1.append('% Of PRO Bonus Credits')
  ...
  for source_no in range(2, source_count + 2):
    ws.cell(row=source_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws.cell(row=source_no, column=(len(column_names_1) - 2)).font = Font(bold=True)
  ...
  for n in range(2, source_count + 2):
    ws.cell(row=n, column=len(column_names_1) - 1).value = "=({}{}/{}{})".format(total_column_letter, n,
                                                                               total_column_letter, source_count + 2)

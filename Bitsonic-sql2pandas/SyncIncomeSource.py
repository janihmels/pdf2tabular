...
def syncincomesourcexrevxhalf(database, df, filename):
...
#Find number of income types
  find_income_type = '''SELECT Normalized_Income_Type_II_9LC, Normalized_Source_9LC 
                        FROM Master WHERE Normalized_Income_Type_9LC = "Synchronisation" GROUP BY Normalized_Income_Type_II_9LC, Normalized_Source_9LC'''
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Income Type,Source x Rev x {}".format(period_name)
  ...
  select_table_1_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                   AS `Total`
                   FROM Master WHERE Statement_Period_Half_9LC <> "" AND Normalized_Income_Type_9LC = "Synchronisation"
                   GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC ORDER BY `Total` DESC'''.format(cut_off)
  ...
  for income_no in range(2, income_type_count + 2):
    ...
    ws.cell(row=income_no, column=3).alignment = Alignment(horizontal="left", vertical="center")
  ...
  for (l, m) in zip(range(4, len(column_names_1) - 1), column_letters[3:]):
    ...
    ws.cell(row=income_type_count + 2, column=l).style = 'total_style'
...
#incomexrevxhalf('DJ Battlecat_616c91a05f278b92afbaa5ae')
#incomexrevxhalf('The Chainsmokers - Hipgnosis - Final_61df0d95f5592bbb47ad9a87')

...
def songxcontractidxrevxhalf(database, df, filename):
...
#Create list of column names
  year_column_names = ['Song Title', 'Third Party', 'Contract ID']
  ...
  half_column_names = ['Song Title', 'Third Party', 'Contract ID']
  ...
  quarter_column_names = ['Song Title', 'Third Party', 'Contract ID']
...
#Build first table
  if mojo:
    ...
    if third_party_type_list[0][0] == "":
      ws.title = "Song x Third Party x Contract ID x Rev x Half"
    else:
      ws.title = "({}) Song x Third Party x Contract ID x Rev x Half".format(third_party_type_list[0][0])
    ...
    for s in all_songs:
      ...
      select_table_1 = '''SELECT Song_Name_9LC, Third_Party_9LC, Contract_ID_9LC,'''
      select_table_2 = ""
      ...
      select_table_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                          FROM Master WHERE Song_Name_9LC = "{}" AND Rights_Type_9LC = "{}" 
                          GROUP BY Song_Name_9LC, Third_Party_9LC, Contract_ID_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off,s_string, third_party_type_list[0][0])
      select_table = select_table_1 + select_table_2 + select_table_3
      mycursor.execute(select_table)
      table = mycursor.fetchall()
      song_table = column_names_final + table
    ...
    #Add total row
      ws.cell(row=total_row_no, column=3).value = 'Total'
      for n in range(4,total_column_no):
    ...
    #Format total row header
      ws.cell(row=total_row_no, column=3).fill = PatternFill("solid", fgColor="A6ACAF")
      ws.cell(row=total_row_no, column=3).font = Font(bold=True)
      ws.cell(row=total_row_no, column=3).alignment = Alignment(horizontal="center", vertical="center")
    ...
    #Format total row and column
      ...
      for f in range(4, total_column_no):
  ...
  #Build rest of tables
    for i in third_party_type_list[1:]:
      ws_new = wb.create_sheet(title="({}) Song x Income x Rev x Half".format(i[0]))
      ws = ws_new
    ...
    #Build song tables
      ...
      for s in all_songs:
        ...
        select_table_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                              FROM Master WHERE Song_Name_9LC = "{}" AND Rights_Type_9LC = "{}" 
                              GROUP BY Song_Name_9LC, Third_Party_9LC, Contract_ID_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off,
                                                                                                                 s_string,
                                                                                                                 i[0])
        select_table = select_table_1 + select_table_2 + select_table_3
        mycursor.execute(select_table)
...
#Create combined sheet
    ...
    ws_1 = wb.active
    ws_1.title = 'Song x Third Party x Contract ID x Half'
    ...
    #Create list of column names
    column_names_1 = ['Song Title', 'Third Party', 'Contract ID']
    ...
    for s in all_songs_combined:
      print(s)
      s_string = escape_string(s)
      select_table_1_1 = '''SELECT Song_Name_9LC, Third_Party_9LC, Contract_ID_9LC,'''
      select_table_1_2 = ""
      for j,k in zip(year_statement_list, statement_list):
      ...
      select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                            ...
                            GROUP BY Song_Name_9LC, Third_Party_9LC, Contract_ID_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off, s_string)
      ...
      #Add total row
      ws_1.cell(row=total_row_no, column=3).value = 'Total'
      for n in range(4, total_column_no):
      ...
      #Format total row header
      ws_1.cell(row=total_row_no, column=3).fill = PatternFill("solid", fgColor="A6ACAF")
      ws_1.cell(row=total_row_no, column=3).font = Font(bold=True)
      ws_1.cell(row=total_row_no, column=3).alignment = Alignment(horizontal="center", vertical="center")

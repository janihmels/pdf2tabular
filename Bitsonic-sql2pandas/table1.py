...
for j in statement_period_half:
  select_table_2 += (''' sum( CASE WHEN Statement_Period_Half_9LC = "{}" AND Song_Name_9LC <> "Pool Revenue" 
                         THEN Royalty_Payable_SB ELSE NULL END) AS `{}`,'''.format(j,j))

select_table_3 = '''sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" THEN Royalty_Payable_SB ELSE NULL END) AS `Total`
...
pool_rev_1 = '''SELECT Song_Name_9LC,'''

pool_rev_2 = ""
for l in statement_period_half:
  pool_rev_2 += ('''sum( CASE WHEN Statement_Period_Half_9LC = "{}" AND Song_Name_9LC = "Pool Revenue"
                    THEN Royalty_Payable_SB ELSE NULL END) AS `{}`,'''.format(l,l))

pool_rev_3 = '''sum( CASE WHEN Song_Name_9LC = "Pool Revenue" THEN Royalty_Payable_SB ELSE NULL END) AS `Total` 
              FROM Master WHERE Song_Name_9LC = "Pool Revenue"
              GROUP BY Song_Name_9LC ORDER BY `Total` DESC'''

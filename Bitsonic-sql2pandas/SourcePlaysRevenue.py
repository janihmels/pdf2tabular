...
def sourcexplaysxrevenue(database, df, filename):
...
#Find number of source types
  find_source_type = '''SELECT Normalized_Source_9LC
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Normalized_Source_9LC'''
...
#Build main pivot table
  ...
  #Worksheet title
  ws.title = "Source x Plays x {}".format(period_name)

  #Build table
  select_table_1 = '''SELECT Normalized_Source_9LC, sum( CASE WHEN Normalized_Source_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Plays_SB ELSE "" END) AS `Plays`,
  sum( CASE WHEN Normalized_Source_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Revenue` FROM Master 
  WHERE Normalized_Source_9LC <> "Pool Revenue" GROUP BY Normalized_Source_9LC ORDER BY `Revenue` desc '''.format(cut_off,cut_off)
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  select_table_2 = '''SELECT Normalized_Source_9LC, sum( CASE WHEN Normalized_Source_9LC = "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Plays_SB ELSE "" END) AS `Plays`,
    sum( CASE WHEN Normalized_Source_9LC = "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Revenue` FROM Master 
    WHERE Normalized_Source_9LC = "Pool Revenue" GROUP BY Song_Name_9LC '''.format(cut_off,cut_off)
  mycursor.execute(select_table_2)
  table_2 = mycursor.fetchall()
  final_table_1 = table_1 + table_2

  #Create list of column names
  column_names_1 = ['Source', 'Plays', 'Revenue','Revenue Per Play']
  ...
  for song_no in range(2, source_count + 2):
    ...
    ws.cell(row=song_no, column=2).number_format = '#,##0'

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1)), column_letters_2):
  ...
  ws.cell(row=source_count + 2, column=2).number_format = '#,##0'
  ...
  #Add revenue per play
  total_column_letter = column_letters[len(column_names_1) - 3]
  for n in range(2, source_count + 2):
    ws.cell(row=n, column=len(column_names_1)).value = "=IFERROR(C{}/B{},0)".format(n, n)
    ws.cell(row=n, column=len(column_names_1)).number_format = '0.00000'

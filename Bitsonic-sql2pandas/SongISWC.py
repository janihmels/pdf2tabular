...
def songISWC(database, df, filename):
...
#Sheet 2: Combined sheet
  ...
  #Build table
  select_table_1_1 = '''SELECT Song_Name_9LC, SONGID_ISWC_SB, sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC <> "Pool Revenue" 
                   GROUP BY Song_Name_9LC, SONGID_ISWC_SB ORDER BY `Total` DESC'''
  select_table_1 = select_table_1_1
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()

  #Add pool revenue line at bottom
  pool_rev_1_1 = '''SELECT Song_Name_9LC, SONGID_ISWC_SB,  sum( CASE WHEN Song_Name_9LC = "Pool Revenue" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC = "Pool Revenue" 
                GROUP BY Song_Name_9LC, SONGID_ISWC_SB ORDER BY `Total`'''
  ...
  #Create list of column names
  column_names_1 = ['Song Title','ISWC', 'Total Revenue']

...
def artistxrevxhalf(database, df, filename):
...
#Create sheets
  wb = Workbook()
  ws = wb.active
  #ws_1 = wb.create_sheet(title="S Artist x Rev x Half", index=0)
  ws_1 = wb.create_sheet(title='S Artist x Rev x Half', index=0)
...
#Create list of column names
  year_column_names = ['Release Artist']
  ...
  half_column_names = ['Release Artist']
  ...
  quarter_column_names = ['Release Artist']
...
#Build main pivot table
  ws.title = "M Artist x Rev x Half"
  total_row_no = -2
  for s in third_party_type_list:
    ...
    select_table_1 = '''SELECT Release_Artist_9LC,'''
    select_table_2 = ""
    for j,k in zip(year_statement_list, statement_list):
      select_table_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Release_Artist_9LC <> "" 
                             THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j,mySQL_column,k,j,k))
    select_table_3 = '''sum( CASE WHEN {} >= "{}" AND Release_Artist_9LC <> ""
                     THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                     FROM Master WHERE Release_Artist_9LC <> ""
                     AND Rights_Type_9LC = "{}"
                     GROUP BY Release_Artist_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off, s[0])
  ...
  #Add line for unknown artists
    unknown_artists_1 = '''SELECT Release_Artist_9LC,'''
    unknown_artists_2 = ""
    for l,m in zip(year_statement_list, statement_list):
      unknown_artists_2 += ('''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}"
                               AND Release_Artist_9LC = ""
                               THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(l,mySQL_column,m,l,m))
    unknown_artists_3 = '''sum( CASE WHEN Release_Artist_9LC = "" AND {} >= "{}"
                           THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                           FROM Master WHERE Release_Artist_9LC = ""
                           AND Rights_Type_9LC = "{}"
                           GROUP BY Release_Artist_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off, s[0])
    unknown_artists = unknown_artists_1 + unknown_artists_2 + unknown_artists_3
    mycursor.execute(unknown_artists)
    unknown_artist_line = mycursor.fetchall()
    final_table = table + unknown_artist_line
  ...
  #Add table to worksheet
    ...
    ws.cell(row=total_row_no, column=1).value = 'Unknown Artists'
...
#Sheet 2: Combined sheet
  ...
  #Add table to worksheet
  table_rows = ws_1[2:artist_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      if k == 0:
        newstr = ILLEGAL_CHARACTERS_RE.sub(r'',j[k])
        cell.value = newstr
      else:
        cell.value = j[k]
      cell.style = 'Comma'
  ...
  ws_1.cell(row=artist_count + 1, column=1).value = 'Unknown Artists'

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters_2):
    ws_1.cell(row=artist_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, artist_count + 1)
    ws_1.cell(row=artist_count + 2, column=l).style = 'Comma'
    ws_1.cell(row=artist_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_1.cell(row=artist_count + 2, column=l).font = Font(bold=True)
  ws_1.cell(row=artist_count + 2, column=1).value = 'Total'
  ws_1.cell(row=artist_count + 2, column=1).font = Font(bold='True')
  ws_1.cell(row=artist_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_1.cell(row=artist_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")
  ...
  for n in range(2, artist_count + 2):
    ws_1.cell(row=n, column=len(column_names_1) - 1).value = "=({}{}/{}{})".format(total_column_letter, n,
                                                                               total_column_letter, artist_count + 2)
...
artistxrevxhalf('Kuya_615e052aecd455245e6954f2', './output_files/output.xlsx')

...
def syncxsourcexrevxhalf(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Sync,Source'

#Sheet 1: Income x Rev x Half
  #Find number of income types
  find_income_type = '''SELECT Normalized_Income_Type_9LC, Normalized_Source_9LC
                        FROM Master WHERE Normalized_Income_Type_9LC = "Sync" GROUP BY Normalized_Income_Type_9LC, Normalized_Source_9LC'''
  mycursor.execute(find_income_type)
  income_type = [i[0] for i in mycursor.fetchall()]
  income_type_count = len(income_type)

  #Build table
  select_table_1_1 = '''SELECT Normalized_Income_Type_9LC, Normalized_Source_9LC,'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                     ...
                     GROUP BY Normalized_Income_Type_9LC, Normalized_Source_9LC ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  ...
  #Column names
  column_names_1 = ['Normalized Income Type', 'Normalized Source']
...
#hipsyncsource('Hipgnosis - Emile Haynie (Combined)_6216baa29e34b2a3f1a4f5e8')

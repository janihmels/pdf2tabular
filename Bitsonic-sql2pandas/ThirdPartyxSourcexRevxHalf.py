...
def thirdpartyxsourcexrevxhalf(database, df, filename):
  mycursor = pandas_cursor(df=df)

#Create sheets
  wb = Workbook()
  ws = wb.active
  ws.title = "3rd Party x Source x Rev x Half"
...
#Find third parties for each type
  ...
  for a in type_list:
    ...
    find_third_party_type = '''SELECT Third_Party_9LC FROM Master WHERE Rights_Type_9LC = "{}" GROUP BY Third_Party_9LC'''.format(a)
...
#Publishing list
  find_publishers = '''SELECT Third_Party_9LC FROM Master WHERE Is_PRO_SB = 0 GROUP BY Third_Party_9LC'''
...
#Create list of column names
  year_column_names = ['Payor', 'Source']
  ...
  half_column_names = ['Payor', 'Source']
  ...
  quarter_column_names = ['Payor', 'Source']
  ...
  for r in third_party_type_list:
    for s in r[1:]:
      ...
      for j,k in zip(year_statement_list, statement_list):
        select_table_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Normalized_Source_9LC <> "Pool Revenue"
      ...
      select_table_3 = '''sum( CASE WHEN Normalized_Source_9LC <> "Pool Revenue" AND 
                          ...
                          Third_Party_9LC = "{}"
                          GROUP BY Third_Party_9LC, Normalized_Source_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off,s)
      select_table = select_table_1 + select_table_2 + select_table_3
      mycursor.execute(select_table)
      table = mycursor.fetchall()

      pool_rev_1 = '''SELECT Third_Party_9LC,
                      Normalized_Source_9LC,'''
      pool_rev_2 = ""
      for l,m in zip(year_statement_list, statement_list):
        pool_rev_2 += ('''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Normalized_Source_9LC = "Pool Revenue"
      ...
      third_party_table = column_names_final + table + pool_revenue
  ...
  #Add tables to sheet
      ...
      for (row,l) in zip(third_party_table_rows, third_party_table):
        for (cell, m) in zip(row, range(len(l))):
          if m == 0 or m == 1:
            escaped_value  = escape_string(l[m])
...
#thirdpartyxsourcexrevxhalf('DJ Battlecat_616c91a05f278b92afbaa5ae')
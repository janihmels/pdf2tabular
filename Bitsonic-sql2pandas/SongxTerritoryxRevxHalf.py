...
def songxterritoryxrevxhalf(database, df, filename):
...
#Find whether quarterly and monthly exist
  ...
  for a in third_party_list:
    ...
    find_statement_number_months = '''SELECT Statement_Period_9LC, Month_Statement_9LC FROM Master WHERE Third_Party_9LC = "{}" GROUP BY Statement_Period_9LC, Month_Statement_9LC'''.format(a)
    ...
    find_months_test = '''SELECT Month_Statement_9LC FROM Master WHERE Month_Statement_9LC <> "" AND Third_Party_9LC = "{}" GROUP BY Month_Statement_9LC'''.format(base_year_value, a)
...
#Column names
  column_names = ['Song Title', 'Territory']
  for k in statement_period_list:
    column_names.append(k)
  column_names.append('Total')
  column_names.append('% Of Revenue')
  column_names.append('Cumulative %')
  column_names_final = [(column_names)]
  print(column_names)

#Build tables
  ws.title = "Song x Territory x Rev x {}".format(period_name)
  total_row_no = 0
  ...
  for s in all_songs:
    ...
    select_table_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                          FROM Master WHERE Song_Name_9LC = "{}" AND Statement_Period_Half_9lC <> ""
                          GROUP BY Song_Name_9LC, Territory_SB ORDER BY `Total` DESC'''.format(cut_off,
                                                                                                             s_string)
...
#songxterritoryxrevxhalf('Edward Henson_61c49dfac94b68a289900e87')
#songxterritoryxrevxhalf('Aleem Smith_61b77345c94b68a289900e80')
...
def releaseyearxrevxhalf(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Release Year x Rev x {}'.format(period_name)

#Sheet 1: Release Year x Rev x Half
  #Find number of sources
  find_source_type = '''SELECT RIGHT( Release_Date_9LC, 4)
                        FROM Master GROUP BY RIGHT( Release_Date_9LC, 4)'''
  mycursor.execute(find_source_type)
  sources = [i[0] for i in mycursor.fetchall()]
  source_count = len(sources)

  #Build table
  select_table_1_1 = '''SELECT RIGHT( Release_Date_9LC, 4),'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                       ...
                       FROM Master 
                       GROUP BY RIGHT( Release_Date_9LC, 4) ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)

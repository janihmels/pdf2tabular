...
def songxnetgross(database, df, filename):
...
#Build main pivot table
  ...
  #Worksheet title
  ws.title = "Song,Net Total,Gross Total"

  #Build table
  select_table_1 = '''SELECT Song_Name_9LC, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Net Total`,
                      sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Payee_Gross_Royalty_Amount_SB ELSE "" END) AS `Gross Total` FROM Master WHERE Song_Name_9LC <> "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC ORDER BY `Net Total` DESC'''.format(cut_off,
                                                                                                                cut_off)
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()

  #Add pool revenue line at bottom
  pool_rev_1 = '''SELECT Song_Name_9LC, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Net Total`,
                      sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Payee_Gross_Royalty_Amount_SB ELSE "" END) AS `Gross Total` FROM Master WHERE Song_Name_9LC = "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC ORDER BY `Net Total` DESC'''.format(cut_off,
                                                                                                                cut_off)
  ...
  for n in range(2, song_count+2):
    ws.cell(row=n, column=4).value = '=B{}/C{}'.format(n,n)

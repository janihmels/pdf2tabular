...
def thirdpartysongcomposer(database, df, filename):
...
#Find number of songs
  find_songs = '''SELECT Third_Party_9LC, Song_Name_9LC, Composer_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Third_Party_9LC, Song_Name_9LC, Composer_SB '''
...
#Build sheet
  #Worksheet title
  ws.title = "Third Party, Song, Composer"

  #Build table
  select_table_1 = '''SELECT Third_Party_9LC, sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" THEN Adjusted_Royalty_SB ELSE "" END)
                        AS `Total Revenue`, 
                        COUNT( IF (Song_Name_9LC <> "Pool Revenue",1,NULL)) `Number of Lines`,
                        Song_Name_9LC, Composer_SB FROM Master WHERE Song_Name_9LC <> "Pool Revenue"
                        GROUP BY Third_PArty_9LC, Song_Name_9LC, Composer_SB'''
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()

  pool_rev_1 = '''SELECT Third_Party_9LC, sum( CASE WHEN Song_Name_9LC = "Pool Revenue" THEN Adjusted_Royalty_SB ELSE "" END)
                          AS `Total Revenue`, 
                          COUNT( IF (Song_Name_9LC = "Pool Revenue",1,NULL)) `Number of Lines`,
                          Song_Name_9LC, Composer_SB FROM Master WHERE Song_Name_9LC = "Pool Revenue"
                          GROUP BY Third_PArty_9LC, Song_Name_9LC, Composer_SB'''
  ...
  column_names_1 = ['Third Party','Total Revenue', 'Line Count', 'Song Title','Composer']
  ...
  for song_no in range(2, song_count + 2):
    ...
    ws.cell(row=song_no, column=3).number_format = '#,##0'
    ws.cell(row=song_no, column=4).alignment = Alignment(horizontal="left", vertical="center")
    ws.cell(row=song_no, column=5).alignment = Alignment(horizontal="left", vertical="center")

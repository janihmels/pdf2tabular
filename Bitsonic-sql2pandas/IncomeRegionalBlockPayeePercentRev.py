...
def incomeregionalblockpayeepercentrev(database, df, filename):
...
#Find number of songs
  find_songs = '''SELECT Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Regional_Block_SB,Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB'''
...
#Build main pivot table
  ...
  #Worksheet title
  ws.title = "Song,Block,Income,Payee %,Rev"

  #Build table
  select_table_1 = '''SELECT Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC <> "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB ORDER BY `Total` DESC'''.format(cut_off)
  ...
  pool_rev_1 = '''SELECT Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC = "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Percent_Received_SB'''.format(cut_off)

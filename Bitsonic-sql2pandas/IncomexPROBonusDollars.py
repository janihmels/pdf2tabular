...
def incomexPRObonusdollarsxhalf(database, df, filename):
...
#Create sheets
  wb = Workbook()
  ws = wb.active
  ws.title = 'Income x PRO Bonus Dollars x Half'
...
#Build sheet
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                   ...
                   FROM Master
                   GROUP BY Normalized_Royalty_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off)

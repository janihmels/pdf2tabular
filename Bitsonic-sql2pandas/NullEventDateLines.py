...
def nulleventdatelines(database, df, filename):
...
#Build sheet
  ...
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' COUNT( IF( (Event_Start_DD IS NULL OR Event_End_DD IS NULL) AND Year_Statement_9LC = "{}" AND {} = "{}",1,NULL)) `{} {} Null Lines`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''COUNT( IF ((Event_Start_DD IS NULL OR Event_End_DD IS NULL) AND {} >= "{}",1,NULL)) `Total Null Lines`
  ...
  select_table_2_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_2_2 += (''' COUNT( IF( (Event_Start_DD IS NOT NULL AND Event_End_DD IS NOT NULL) AND Year_Statement_9LC = "{}" AND {} = "{}",1,NULL)) `{} {} Non Null Lines`,'''.format(j, mySQL_column, k, j, k))
  select_table_2_3 = '''COUNT( IF ( (Event_Start_DD IS NOT NULL AND Event_End_DD IS NOT NULL) AND {} >= "{}",1,NULL)) `Total Non Null Lines`
                   FROM Master WHERE {} <> ""'''.format(cut_off_field, cut_off,cut_off_field)
  ...
  for a, b, c, d in zip(range(2,len(column_names_1)+2),table_1[0], table_2[0], column_names_1):
    ...
    ws.cell(row=2, column=1).value = 'No. Null Lines'
    ws.cell(row=3, column=1).value = 'No. Non-Null Lines'
    ws.cell(row=2, column=a).number_format = '#,##0'
    ws.cell(row=3, column=a).number_format = '#,##0'
  for e in range(2, 4):
  ...
  for f, g in zip(range(2,len(column_names_1)+1),column_letters[1:]):
    ...
    ws.cell(row=4, column=f).number_format = '#,##0'
  ...
  ws.cell(row=7, column=1).value = '% Null Lines'
  ws.cell(row=8, column=1).value = '% Non-Null Lines'
...
#nulleventdatelines('Tempo - Lukas Graham_61f33d1b2c528a75ab542024')

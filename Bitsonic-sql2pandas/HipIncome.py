...
def hipincome(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Income Type'
  ws_1 = wb.create_sheet(title='Income Type II')
  ws_2 = wb.create_sheet(title='Income Type,Income Type II')
  ws_3 = wb.create_sheet(title='Song,Income')
  ws_4 = wb.create_sheet(title='Income,Source')
...
#Sheet 1: Income x Rev x Half
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                     AS `Total`
                     FROM Master 
                     GROUP BY Normalized_Income_Type_9LC ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  ...
  #Column names
  column_names_1 = ['Normalized Income Type']
  ...
  for song_no in range(2, income_type_count + 2):
...
#Sheet 2: Income II x Rev x Half
  #Find number of income types
  find_income_type = '''SELECT Normalized_Income_Type_II_9LC 
                        FROM Master GROUP BY Normalized_Income_Type_II_9LC'''
  mycursor.execute(find_income_type)
  income_type = [i[0] for i in mycursor.fetchall()]
  income_type_count = len(income_type)

  #Build table
  select_table_1_1 = '''SELECT Normalized_Income_Type_II_9LC,'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}"
                             THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                     AS `Total`
                     FROM Master 
                     GROUP BY Normalized_Income_Type_II_9LC ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  final_table_1 = mycursor.fetchall()

  #Column names
  column_names_1 = ['Normalized Income Type II']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
...
#Sheet 3: Income,Income II x Rev x Half
  #Find number of income types
  find_income_type = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC 
                        FROM Master GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC'''
  mycursor.execute(find_income_type)
  income_type = [i[0] for i in mycursor.fetchall()]
  income_type_count = len(income_type)
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                     AS `Total`
                     FROM Master 
                     GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  final_table_1 = mycursor.fetchall()
  ...
  for song_no in range(2, income_type_count + 2):
    ...
    ws_2.cell(row=song_no, column=(len(column_names_1) - 2)).font = Font(bold=True)
...
#Sheet 4: Song,Income Type x Rev x Half
  ...
  column_names_1 = ['Song Title', 'Normalized Income Type']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
  ...
  #Build tables
  total_row_no = 0
  ws_3.insert_cols(1, len(column_names_1))
  for s in all_songs_combined:
    ...
    ws_3.append(range(len(song_table_1) + 2))
    ...
    for o in range(header_row_no + 1, total_row_no):
      ...
      ws_3.cell(row=o, column=total_column_no).number_format = '0.00%'
    ...
    ws_3.cell(row=header_row_no + 1, column=total_column_no + 1).value = '={}{}'.format(percentage_column_letter,
                                                                                        header_row_no + 1)
    ws_3.cell(row=header_row_no + 1, column=total_column_no + 1).number_format = '0.00%'
    ...
    for p in range(header_row_no + 2, total_row_no):
      ...
      ws_3.cell(row=p, column=total_column_no + 1).number_format = '0.00%'
    ...
    for e in range(header_row_no + 1, total_row_no):
      ws_3.cell(row=e, column=total_column_no - 1).font = Font(bold=True)
...
#Sheet 5: Income Type By Source
  #Find number of songs
  find_songs = '''SELECT Normalized_Income_Type_9LC, sum( Adjusted_Royalty_SB) 
                  FROM Master WHERE Song_Name_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" 
                  GROUP BY  Normalized_Income_Type_9LC ORDER BY sum( Adjusted_Royalty_SB) DESC'''.format(standard_cut_off)
  mycursor.execute(find_songs)
  songs = [i[0] for i in mycursor.fetchall()]
  find_pool_rev = '''SELECT Normalized_Income_Type_9LC, sum( Adjusted_Royalty_SB)
                     FROM Master WHERE Song_Name_9LC = "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" 
                     GROUP BY Normalized_Income_Type_9LC'''.format(standard_cut_off)
  mycursor.execute(find_pool_rev)
  pool_rev = [i[0] for i in mycursor.fetchall()]
  all_songs_combined = songs + pool_rev

  #Create list of column names
  column_names_1 = ['Normalized Income Type', 'Source']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
  ...
  #Build tables
  total_row_no = 0
  ws_4.insert_cols(1, len(column_names_1))
  for s in all_songs_combined:
    s_string = escape_string(s)
    select_table_1_1 = '''SELECT Normalized_Income_Type_9LC,
    ...
    for j, k in zip(year_statement_list, statement_list):
      select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}"
                                 THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j,
                                                                                             k))
    select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                            FROM Master WHERE Normalized_Income_Type_9LC = "{}"
                            GROUP BY Normalized_Income_Type_9LC,
                            Normalized_Source_9LC ORDER BY `Total` DESC'''.format(
      cut_off_field, cut_off, s_string)
    select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
    mycursor.execute(select_table_1)
    table_1 = mycursor.fetchall()
    ...
    header_row_no = total_row_no - len(song_table_1)
    total_column_no = len(column_names_1) - 1
    ws_4.append(range(len(song_table_1) + 2))
    song_table_rows = ws_3[header_row_no:space_row_no]
    for (row, l) in zip(song_table_rows, song_table_1):
      for (cell, m) in zip(row, range(len(l))):
        cell.value = l[m]
    ...
    #Add total row
    ws_4.cell(row=total_row_no, column=2).value = 'Total'
    for n in range(3, total_column_no):
      ws_4.cell(row=total_row_no, column=n).value = '=SUM({}{}:{}{})'.format(column_letters[n - 1], header_row_no + 1,
                                                                             column_letters[n - 1], total_row_no - 1)
    ...
    cumulative_column_letter = column_letters[len(column_names_1) - 1]
    for o in range(header_row_no + 1, total_row_no):
      ws_4.cell(row=o, column=total_column_no).value = '={}{}/{}{}'.format(total_column_letter, o,
                                                                           total_column_letter, total_row_no)
      ws_4.cell(row=o, column=total_column_no).number_format = '0.00%'
      ws_4.cell(row=o, column=total_column_no).font = Font(name="Calibri", size="11")
    ws_4.cell(row=header_row_no + 1, column=total_column_no + 1).value = '={}{}'.format(percentage_column_letter,
                                                                                        header_row_no + 1)
    ws_4.cell(row=header_row_no + 1, column=total_column_no + 1).number_format = '0.00%'
    ws_4.cell(row=header_row_no + 1, column=total_column_no + 1).font = Font(name="Calibri", size=11)
    for p in range(header_row_no + 2, total_row_no):
      ws_4.cell(row=p, column=total_column_no + 1).value = '={}{}+{}{}'.format(cumulative_column_letter, p - 1,
                                                                               percentage_column_letter, p)
      ws_4.cell(row=p, column=total_column_no + 1).number_format = '0.00%'
      ws_4.cell(row=p, column=total_column_no + 1).font = Font(name="Calibri", size=11)

    #Format header row
    for b in range(1, len(column_names_1) + 1):
      ws_4.cell(row=header_row_no, column=b).fill = PatternFill("solid", fgColor="A6ACAF")
      ws_4.cell(row=header_row_no, column=b).alignment = Alignment(horizontal="center", vertical="center")
      ws_4.cell(row=header_row_no, column=b).font = Font(bold=True)

    #Format numbers
    for c in range(3, total_column_no):
      for d in range(header_row_no + 1, total_row_no + 1):
        ws_4.cell(row=d, column=c).style = 'Comma'
        ws_4.cell(row=d, column=c).font = Font(name="Calibri", size="11")

    #Format total row header
    ws_4.cell(row=total_row_no, column=2).fill = PatternFill("solid", fgColor="A6ACAF")
    ws_4.cell(row=total_row_no, column=2).font = Font(bold=True)
    ws_4.cell(row=total_row_no, column=2).alignment = Alignment(horizontal="center", vertical="center")

    #Format total row and column
    for e in range(header_row_no + 1, total_row_no):
      ws_4.cell(row=e, column=total_column_no - 1).font = Font(bold=True)
    for f in range(3, total_column_no):
      ws_4.cell(row=total_row_no, column=f).font = Font(bold=True)

    #Insert row between tables
    ws_4.insert_rows(total_row_no + 1)
    total_row_no += 1

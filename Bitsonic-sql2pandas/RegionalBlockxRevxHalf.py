...
def regionalblockxrevxhalf(database, df, filename):
...
#Find number of regions
  find_regional_blocks = '''SELECT Regional_Block_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Regional_Block_SB'''
  mycursor.execute(find_regional_blocks)
  regional_blocks = [i[0] for i in mycursor.fetchall()]
  regional_block_count = len(regional_blocks)
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Regional Block x Rev x {}".format(period_name)

  #Build table
  select_table_1_1 = '''SELECT Regional_Block_SB,'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}"
                   ...
                   GROUP BY Regional_Block_SB ORDER BY `Total` DESC'''.format(cut_off)
  ...
  for column_no in range(1, len(column_names_1) + 1):
    for row_no in range(1, regional_block_count + 1):
  ...
  table_rows = ws[2:regional_block_count + 1]
  ...
  for region_no in range(2, regional_block_count + 2):
    ws.cell(row=region_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws.cell(row=region_no, column=(len(column_names_1) - 2)).font = Font(bold=True)

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters_2):
    ws.cell(row=regional_block_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, regional_block_count + 1)
    ws.cell(row=regional_block_count + 2, column=l).style = 'Comma'
    ws.cell(row=regional_block_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=regional_block_count + 2, column=l).font = Font(bold=True)
  ws.cell(row=regional_block_count + 2, column=1).value = 'Total'
  ws.cell(row=regional_block_count + 2, column=1).font = Font(bold='True')
  ws.cell(row=regional_block_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws.cell(row=regional_block_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

  #Add % of revenue column
  total_column_letter = column_letters[len(column_names_1) - 3]
  for n in range(2, regional_block_count + 2):
    ws.cell(row=n, column=len(column_names_1) - 1).value = "=({}{}/{}{})".format(total_column_letter, n,
                                                                               total_column_letter, regional_block_count + 2)
  ...
  for o in range(3, regional_block_count + 2):
...
#regionalblockxrevxhalf('The Chainsmokers - Hipgnosis - Final_61df0d95f5592bbb47ad9a87')

...
def creditsVdollars(database, df, filename):
...
#Create sheets
  wb = Workbook()
  ws = wb.active
  ws.title = 'PRO Bonus Credits V Dollars'
...
#Build sheet
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                   ...
                   FROM Master ORDER BY `Total` DESC'''.format(cut_off_field,cut_off)
  ...
  for column_no in range(1, len(column_names_1) + 1):
    for row_no in range(1, bonus_type_count + 1):
  ...
  table_rows = ws[2:bonus_type_count + 1]
  for j, k, l in zip(range(2,len(column_names_1)+1),table_1[0],table_2[0]):
      ws.cell(row=2, column=j).value = k
      ws.cell(row=2, column=j).style = 'Comma'
      ws.cell(row=2, column=j).alignment = Alignment(horizontal="right", vertical="center")
      ws.cell(row=2, column=j).font = Font(name="Calibri", size="11")
      ws.cell(row=3, column=j).value = l
      ws.cell(row=3, column=j).style = 'Comma'
      ws.cell(row=3, column=j).alignment = Alignment(horizontal="right", vertical="center")
      ws.cell(row=3, column=j).font = Font(name="Calibri", size="11")
  for song_no in range(2, bonus_type_count + 2):
    ws.cell(row=song_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws.cell(row=song_no, column=len(column_names_1)).font = Font(bold=True)
  ws.cell(row=2, column=1).value = 'PRO Bonus Credits'
  ws.cell(row=3, column=1).value = 'PRO Bonus Dollars'

...
def songdateIDISWCISRCxrevxhalf(database, df, filename):
...
#Find number of songs
  find_songs = '''SELECT Song_Name_9LC, Release_Date_9LC, Payer_Song_ID_SB, SONGID_ISWC_SB, SONGID_ISRC_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Release_Date_9LC, Payer_Song_ID_SB, SONGID_ISWC_SB, SONGID_ISRC_SB '''
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Song,Date,ID,ISWC,ISRC x Rev x {}".format(period_name)
  ...
  select_table_1_3 = '''sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}"
                   ...
                   GROUP BY Song_Name_9LC, Release_Date_9LC,Payer_Song_ID_SB, SONGID_ISWC_SB, SONGID_ISRC_SB ORDER BY `Total` DESC'''.format(cut_off)

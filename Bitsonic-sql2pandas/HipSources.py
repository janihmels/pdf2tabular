...
def hipsources(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Source'
  ws_1 = wb.create_sheet(title='Income,Income II,Source')
  ws_2 = wb.create_sheet(title='(Sync) Income,Income II,Source')

#Sheet 1: Source x Rev x Half
  #Find number of sources
  find_source_type = '''SELECT Normalized_Source_9LC
  ...
  table_rows = ws[2:source_count + 1]
  ...
  for song_no in range(2, source_count + 2):
  ...
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters[1:]):
    ws.cell(row=source_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_count + 1)
    ws.cell(row=source_count + 2, column=l).style = 'Comma'
    ws.cell(row=source_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=source_count + 2, column=l).font = Font(bold=True)
  ws.cell(row=source_count + 2, column=1).value = 'Total'
  ws.cell(row=source_count + 2, column=1).font = Font(bold='True')
  ws.cell(row=source_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws.cell(row=source_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")
...
#Sheet 2: Income,Income II,Source x Rev x Half
  #Find number of sources
  find_source_type = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC
                        FROM Master GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC'''
  mycursor.execute(find_source_type)
  sources = [i[0] for i in mycursor.fetchall()]
  source_count = len(sources)

  #Build table
  select_table_1_1 = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC,'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Normalized_Source_9LC <> "Pool Revenue" 
                               THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''sum( CASE WHEN Normalized_Source_9LC <> "Pool Revenue" AND {} >= "{}"
                       THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                       FROM Master WHERE Normalized_Source_9LC <> "Pool Revenue"
                       GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()

  #Add pool revenue line at bottom
  pool_rev_1_1 = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC,'''
  pool_rev_1_2 = ""
  for l, m in zip(year_statement_list, statement_list):
    pool_rev_1_2 += ('''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Normalized_Source_9LC = "Pool Revenue"
                          THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(l, mySQL_column, m, l, m))
  ...
  #Column names
  column_names_1 = ['Normalized Income Type', 'Normalized Income Type II', 'Normalized Source']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
  ...
  #Add table to worksheet
  table_rows = ws_1[2:source_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      cell.value = j[k]
      cell.style = 'Comma'
  ...
  for (l, m) in zip(range(4, len(column_names_1) - 1), column_letters[3:]):
    ws_1.cell(row=source_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_count + 1)
    ws_1.cell(row=source_count + 2, column=l).style = 'Comma'
    ws_1.cell(row=source_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_1.cell(row=source_count + 2, column=l).font = Font(bold=True)
  ws_1.cell(row=source_count + 2, column=1).value = 'Total'
  ws_1.cell(row=source_count + 2, column=1).font = Font(bold='True')
  ws_1.cell(row=source_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_1.cell(row=source_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")
...
#Sheet 3: (Sync)Income,Income II,Source x Rev x Half
 #Find number of sources
  find_source_type = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC
                        FROM Master WHERE Normalized_Income_Type_9LC = "Synchronisation" GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC'''
  mycursor.execute(find_source_type)
  sources = [i[0] for i in mycursor.fetchall()]
  source_count = len(sources)
  ...
  select_table_1_3 = '''sum( CASE WHEN Normalized_Source_9LC <> "Pool Revenue" AND {} >= "{}"
                       THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                       FROM Master WHERE Normalized_Source_9LC <> "Pool Revenue" AND Normalized_Income_Type_9LC = "Synchronisation"
                       GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  ...
  #Add table to worksheet
  table_rows = ws_2[2:source_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      cell.value = j[k]
      cell.style = 'Comma'
  ...
  #Add total row
  for (l, m) in zip(range(4, len(column_names_1) - 1), column_letters[3:]):
    ws_2.cell(row=source_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_count + 1)
    ws_2.cell(row=source_count + 2, column=l).style = 'Comma'
    ws_2.cell(row=source_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_2.cell(row=source_count + 2, column=l).font = Font(bold=True)
  ws_2.cell(row=source_count + 2, column=1).value = 'Total'
  ws_2.cell(row=source_count + 2, column=1).font = Font(bold='True')
  ws_2.cell(row=source_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_2.cell(row=source_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

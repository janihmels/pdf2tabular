...
def playsxrevxhalf(database, df, filename):
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Plays x Rev x {}".format(period_name)
  ...
  for l, m in zip(year_statement_list, statement_list):
    pool_rev_1_2 += ('''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Song_Name_9LC = "Pool Revenue"
                      THEN Plays_SB ELSE "" END) AS `{} {}`,'''.format(l,mySQL_column,m, l,m))
  ...
  column_names_1.append('% Of Plays')

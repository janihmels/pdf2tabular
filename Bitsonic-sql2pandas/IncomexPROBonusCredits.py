...
def incomexPRObonuscreditsxhalf(database, df, filename):
...
#Create sheets
  wb = Workbook()
  ws = wb.active
  ws.title = 'Income x PRO Bonus Credits x Half'
...
#Find number of income types
  find_income_types = '''SELECT Normalized_Income_Type_9LC
                  FROM Master GROUP BY Normalized_Income_Type_9LC'''
  mycursor.execute(find_income_types)
...
#Build sheet
  ...
  for n in range(2, income_type_count + 2):
    ws.cell(row=n, column=len(column_names_1) - 1).value = "=({}{}/{}{})".format(total_column_letter, n,
                                                                               total_column_letter, income_type_count + 2)

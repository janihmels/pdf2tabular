...
def sourcechainlines(database, df, filename):
...
#Build main pivot table
  ...
  #Worksheet title
  ws.title = "Source Chain Line Count".format(period_name)

  #Build table
  select_table_1 = '''SELECT TRIM(Source_Chain_SB), COUNT( Source_Chain_SB) AS `Lines` FROM Master 
  GROUP BY TRIM(Source_Chain_SB) ORDER BY `Lines` desc '''.format(cut_off,cut_off)
  ...
  for (l, m) in zip(range(2, len(column_names_1)+1), column_letters_2):
    ...
    ws.cell(row=source_count + 2, column=l).number_format = '#,##0'

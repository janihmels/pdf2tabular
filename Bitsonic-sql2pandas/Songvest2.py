...
def songvest2(database, df, filename):
...
#Current year
  ...
  find_current_year_exist = '''SELECT Year_Statement_9LC FROM Master WHERE Year_Statement_9LC = "{}" GROUP BY Year_Statement_9LC'''.format(current_year)
  mycursor.execute(find_current_year_exist)
  current_year_exist = [i[0] for i in mycursor.fetchall()]
  minus_variable = 4
  graph_add = 2
  if len(current_year_exist) == 0:
    current_year = current_year - 1
    minus_variable = 3
    graph_add = 1

#Find complete years
  find_complete = '''SELECT Quarter_Statement_9LC, Year_Statement_9LC FROM Master WHERE Year_Statement_9LC < "{}" GROUP BY Quarter_Statement_9LC, Year_Statement_9LC'''.format(current_year)
  mycursor.execute(find_complete)
  quarters = [i[0] for i in mycursor.fetchall()]
  no_complete_years = int((len(quarters)-len(quarters)%4)/4)
  summary_years = []
  if no_complete_years == 0:
    find_complete = '''SELECT Half_Statement_9LC, Year_Statement_9LC FROM Master WHERE Year_Statement_9LC < "{}" GROUP BY Half_Statement_9LC, Year_Statement_9LC'''.format(current_year)
    mycursor.execute(find_complete)
...
#Find where quarterly data exists
  find_quarterly = '''SELECT Third_Party_9LC FROM Master WHERE Quarter_Statement_9LC <> ""  GROUP BY Third_Party_9LC'''.format(
      third_party_string)
  mycursor.execute(find_quarterly)
  quarterly = [i[0] for i in mycursor.fetchall()]
  test_quarterly = '''SELECT Quarter_Statement_9LC FROM Master WHERE Quarter_Statement_9LC <> "" AND Year_Statement_9LC = "{}" AND Third_Party_9LC IN ({}) GROUP BY Quarter_Statement_9LC'''.format(
    base_year_value, third_party_string)
  mycursor.execute(test_quarterly)
  quarterly_test = [i[0] for i in mycursor.fetchall()]
  if len(quarterly) != 0 and len(quarterly_test) == 4:
      period = 'quarterly'

#Find where monthly data exists
  find_monthly = '''SELECT Third_Party_9LC FROM Master WHERE Month_Statement_9LC <> "" AND Third_Party_9LC in ({}) GROUP BY Third_Party_9LC'''.format(
      third_party_string)
  mycursor.execute(find_monthly)
  monthly = [i[0] for i in mycursor.fetchall()]
  test_monthly = '''SELECT Month_Statement_9LC FROM Master WHERE Month_Statement_9LC <> "" AND Year_Statement_9LC = "{}" AND Third_Party_9LC IN ({}) GROUP BY Month_Statement_9LC'''.format(base_year_value,third_party_string)
  mycursor.execute(test_monthly)
  monthly_test = [i[0] for i in mycursor.fetchall()]
  if len(monthly) != 0 and len(monthly_test) == 12:
      period = 'monthly'
...
#Find smallest period
  if period == 'monthly':
    ...
    half_month_split = 6
    half_quarter_split = 2

  print(divisions)

  #Find if period data complete
  complete_data_list = [['monthly', 'Month_Statement_9LC',month_statement_list,year_statement_list_month],
                        ['quarterly', 'Quarter_Statement_9LC', quarter_statement_list, year_statement_list_quarter],
                        ['half','Half_Statement_9LC', half_statement_list, year_statement_list_half]]
  '''index = complete_data_list.index(smallest_period)'''
  find_if_complete = '''SELECT DISTINCT Third_Party_9LC FROM Master WHERE {} <> "" GROUP BY Third_Party_9LC'''.format(
      mySQL_column)
  mycursor.execute(find_if_complete)
  if_complete = [i[0] for i in mycursor.fetchall()]
  if len(if_complete) == len(third_party_list):
      data_period_complete = True
  else:
      data_period_complete = False

  for a in complete_data_list:
      if a:
          monthly_third_parties = []
          for f in complete_data_list:
              for e in third_party_list:
                  find_party_monthly = '''SELECT Third_Party_9LC FROM Master WHERE Month_Statement_9LC <> "" AND Third_Party_9LC = "{}"'''.format(
                      e)
                  mycursor.execute(find_party_monthly)
                  party_monthly = [i[0] for i in mycursor.fetchall()]
                  if len(party_monthly) == 1:
                      monthly_third_parties.append(party_monthly)
...
#Find number of sources
  find_sources = '''SELECT Normalized_Source_9LC
                      FROM Master GROUP BY Normalized_Source_9LC'''
  mycursor.execute(find_sources)
  sources = [i[0] for i in mycursor.fetchall()]
...
#Find number of sources
  ...
  income_types = [i[0] for i in mycursor.fetchall()]
...
#Create styles
  ...
  date_style.number_format = 'mm-dd-yy'
...
#Sheet 1: Song List
  ...
  list_sub_tables = []
  for e in third_party_list:
    find_party_monthly = '''SELECT Third_Party_9LC FROM Master WHERE Month_Statement_9LC <> "" AND Third_Party_9LC = "{}" GROUP BY Third_Party_9LC'''.format(e)
    mycursor.execute(find_party_monthly)
    party_monthly = [i[0] for i in mycursor.fetchall()]
    if len(party_monthly) == 1:
      period_index = 0
    find_party_quarterly = '''SELECT Third_Party_9LC FROM Master WHERE Quarter_Statement_9LC <> "" AND Third_Party_9LC = "{}" GROUP BY Third_Party_9LC'''.format(
      e)
    mycursor.execute(find_party_quarterly)
    party_quarterly = [i[0] for i in mycursor.fetchall()]
    if len(party_quarterly) == 1:
      period_index = 1
    else:
      period_index = 2
    ...
    if complete_data_list[period_index][0] == period:
      select_table_1_1 = '''SELECT Song_Name_9LC,'''
      select_table_1_2 = ""
      for c in summary_years:
        select_table_1_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND Third_Party_9LC ="{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `{}`,'''.format(c,e,c)
      for a, b in zip(year_statement_list, statement_list):
        select_table_1_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Third_Party_9LC = "{}"
                                THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(a,mySQL_column,b, e, a,b)
      select_table_1_3 = '''sum( CASE WHEN Year_Statement_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END)
                                  AS `Total` FROM Master GROUP BY Song_Name_9LC ORDER BY `Total` DESC'''.format(cut_off, e)
      select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
      mycursor.execute(select_table_1)
      third_party_table_1 = mycursor.fetchall()
      print(select_table_1)


    if complete_data_list[period_index][0] != period:
      if period == 'monthly':
        if complete_data_list[period_index][0] == 'monthly':
          split = 1
        if complete_data_list[period_index][0] == 'quarterly':
          split = 3
        if complete_data_list[period_index][0] == 'half':
          split = 6
      if period == 'quarterly':
        if complete_data_list[period_index][0] == 'quarterly':
          split = 1
      ...
      select_table_1_2 = ""
      for c in summary_years:
        select_table_1_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND Third_Party_9LC = "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `{}`,'''.format(
          c, e, c)
      for a, b in zip(complete_data_list[period_index][3], complete_data_list[period_index][2]):
        select_table_1_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Third_Party_9LC = "{}"
                                      THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(a, complete_data_list[period_index][1], b, e, a, b)
      select_table_1_3 = '''sum( CASE WHEN Year_Statement_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END)
                                        AS `Total` FROM Master GROUP BY Song_Name_9LC ORDER BY `Total` DESC'''.format(
        cut_off, e)
      select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
      print(select_table_1)
      mycursor.execute(select_table_1)
      initial_third_party_table_1 = mycursor.fetchall()
      third_party_table_1 = []
      third_party_table_line = []
      for a in initial_third_party_table_1:
        third_party_table_line.append(a[0])
        if len(summary_years) != 0:
          for d in a[1: len(summary_years)+1]:
            third_party_table_line.append(d)
        for b in a[1+len(summary_years):len(a)-1]:
          for c in range(split):
            third_party_table_line.append(b/split)
        third_party_table_line.append(a[-1])
        third_party_table_1.append(third_party_table_line)
        third_party_table_line = []

    list_sub_tables.append(third_party_table_1)

  all_columns = summary_years + statement_list
  table_1_element = []
  sub_total = 0
  for v in range(song_count):
    table_1_element.append(third_party_table_1[v][0])
    for u in range(1,len(all_columns)+1):
      for w in list_sub_tables:
        sub_total += w[v][u]
      table_1_element.append(sub_total)
      sub_total = 0
    table_1_element.append(third_party_table_1[v][-1])
    table_1.append(table_1_element)
    table_1_element = []
  ...
  #Size of worksheet
  for column_no in range(2, len(column_names_1)+4):
    for row_no in range(2, len(table_1)+4):
      ws_1.cell(row=row_no, column=column_no)
  print(len(detailed_remainder)/divisions)
  #Title row
  ws_1.cell(row=2, column=2).value = '{}'.format(title_names_1[0])
  ws_1.cell(row=2, column=2).style = 'title_style'
  if first_year_complete:
    for d, e in zip(range(len(summary_years)+1, (len(title_names_1)-minus_variable)*divisions+2, divisions), title_names_1[1+len(summary_years):]):
  ...
  if first_year_complete and len(detailed_remainder)/divisions == 1:
    for k in range(len(summary_years)+1, (len(title_names_1)-minus_variable)*divisions+2, divisions):
    ...
    for n in range(len(summary_years)+1, (len(title_names_1)-minus_variable)*divisions+2, divisions):
...
#Sheet 1.1: (Top 10) Song List
  ...
  if first_year_complete and len(detailed_remainder)/divisions == 1:
    for d, e in zip(range(len(summary_years)+1, (len(title_names_1_1)-minus_variable)*divisions+2, divisions),
  ...
  #Top songs
  top_10_songs = table_1[:10]

  #Other songs
  all_columns = statement_list + summary_years
  other_songs = table_1[10:]
  ...
  if first_year_complete:
    for d, e in zip(range(len(summary_years)+1, (len(title_names_1_2) - minus_variable) * divisions + 2, divisions),
  ...
  #Top songs
  top_5_songs = table_1[:5]

  #Other songs
  all_columns = summary_years + statement_list
  other_songs = table_1[5:]
  other_songs_element = ['Other Songs']
  period_total = 0
  for z in range(1, len(all_columns) + 2):
    for w in other_songs:
  ...
  if first_year_complete:
    ...
    for t in range(len(column_names_1_2) + 2, len(column_names_1_2) + 5):
      ws_1_2.cell(row=2, column=t).border = Border(left=thin, top=thin)
...
#Sheet 2: Source x Rev x Half
  ...
  list_sub_tables = []
  for e in third_party_list:
    find_party_monthly = '''SELECT Third_Party_9LC FROM Master WHERE Month_Statement_9LC <> "" AND Third_Party_9LC = "{}" GROUP BY Third_Party_9LC'''.format(
      e)
    mycursor.execute(find_party_monthly)
    party_monthly = [i[0] for i in mycursor.fetchall()]
    if len(party_monthly) == 1:
    ...
    if complete_data_list[period_index][0] == period:
      ...
      select_table_2_2 = ""
      for c in summary_years:
        select_table_2_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND Third_Party_9LC = "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `{}`,'''.format(c,e,c)
      for a, b in zip(year_statement_list, statement_list):
        select_table_2_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Third_Party_9LC = "{}"
                                THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(a, mySQL_column, b,e, a, b)
      ...
      third_party_table_2 = mycursor.fetchall()
    ...
    if complete_data_list[period_index][0] != period:
      ...
      select_table_2_2 = ""
      for c in summary_years:
        select_table_2_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND Third_Party_9LC = "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `{}`,'''.format(
          c, e, c)
      for a, b in zip(complete_data_list[period_index][3], complete_data_list[period_index][2]):
        select_table_2_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Third_Party_9LC = "{}"
                                      THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(a, complete_data_list[period_index][1], b, e,
                                                                                                  a, b)
      select_table_2_3 = '''sum( CASE WHEN Year_Statement_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END)
      ...
      select_table_2 = select_table_2_1 + select_table_2_2 + select_table_2_3
      mycursor.execute(select_table_2)
      initial_third_party_table_2 = mycursor.fetchall()
      third_party_table_2 = []
      third_party_table_line = []
      for a in initial_third_party_table_2:
        third_party_table_line.append(a[0])
        for d in a[1: len(summary_years) + 1]:
          third_party_table_line.append(d)
        for b in a[1 + len(summary_years):len(a) - 1]:
          for c in range(split):
            third_party_table_line.append(b / split)
        third_party_table_line.append(a[-1])
        third_party_table_2.append(third_party_table_line)
        third_party_table_line = []
    list_sub_tables.append(third_party_table_2)

  table_2_element = []
  sub_total = 0
  for v in range(source_count):
    table_2_element.append(third_party_table_2[v][0])
    for u in range(1, len(statement_list)+len(summary_years)+1):
      for w in list_sub_tables:
        if w[v][u] == None:
            sub_total += 0
        else:
          sub_total += w[v][u]
      table_2_element.append(sub_total)
      sub_total = 0
    table_2_element.append(third_party_table_2[v][-1])
    table_2.append(table_2_element)
    table_2_element = []
  ...
  if first_year_complete:
    for d, e in zip(range(len(summary_years)+1, (len(title_names_2) - minus_variable) * divisions + 2, divisions), title_names_2[1+len(summary_years):]):
  ...
  thin = Side(border_style="thin", color="000000")
  if first_year_complete:
    for k in range(len(summary_years)+1, (len(title_names_2)-minus_variable)*divisions+2, divisions):
    ...
    for n in range(len(summary_years)+1, (len(title_names_2)-minus_variable)*divisions+2, divisions):
...
#Sheet 2.1: (Top 10) Sources
  ...
  #Top sources
  top_sources = table_2[:10]

  #Other sources
  all_columns = summary_years + statement_list
  other_sources = table_2[10:]
  ...
  if first_year_complete:
    for d, e in zip(range(len(summary_years)+1, (len(title_names_2_1) - minus_variable) * divisions + 2, divisions),
...
#Sheet 3: (Top 10) Source List by Third Party
  ...
  for s in third_party_list:
    ...
    # Find where monthly data exists
    find_monthly = '''SELECT Third_Party_9LC FROM Master WHERE Month_Statement_9LC <> "" AND Third_Party_9LC = "{}" GROUP BY Third_Party_9LC'''.format(
      s)
    mycursor.execute(find_monthly)
    monthly = [i[0] for i in mycursor.fetchall()]
    if len(monthly) == 1:
      third_party_period = 'monthly'

    # Find where quarterly data exists
    find_quarterly = '''SELECT Third_Party_9LC FROM Master WHERE Quarter_Statement_9LC <> "" AND Third_Party_9LC = "{}" GROUP BY Third_Party_9LC'''.format(
      s)
    mycursor.execute(find_quarterly)
    quarterly = [i[0] for i in mycursor.fetchall()]
    if len(quarterly) == 1:
      third_party_period = 'quarterly'
    else:
      third_party_period = 'half'

    # Find smallest period
    if third_party_period == 'monthly':
      smallest_period = 'monthly'
      mySQL_column = 'Month_Statement_9LC'
      statement_list = month_statement_list
      year_statement_list = year_statement_list_month
      statement_period_list = statement_period_month_list
    elif third_party_period == 'quarterly':
      smallest_period = 'quarterly'
      mySQL_column = 'Quarter_Statement_9LC'
      statement_list = quarter_statement_list
      year_statement_list = year_statement_list_quarter
    ...
    if len(remainder) == 0:
      find_remainder = '''SELECT {} FROM Master WHERE Year_Statement_9LC = "{}" AND {} <> "" GROUP BY {}'''.format(
        mySQL_column, all_years[-1], mySQL_column, mySQL_column)
      mycursor.execute(find_remainder)
      remainder = [i[0] for i in mycursor.fetchall()]
    ...
    if first_year_complete:
      for d, e in zip(range(len(summary_years)+1, (len(title_names_3) - minus_variable) * divisions + 2, divisions), title_names_3[1 + len(summary_years):]):
...
#Sheet 3.1: (Top 10) Source List by Third Party
  ...
  for s in third_party_list:
    ...
    if first_year_complete:
      for d, e in zip(range(len(summary_years)+1, (len(title_names_3_1) - minus_variable) * divisions + 2, divisions),
...
#Sheet 4: Revenue Over Time
  ...
  for e in third_party_list:
    ...
    if complete_data_list[period_index][0] == period:
      ...
      select_table_4_2 = ""
      for c in summary_years:
        select_table_4_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND Third_Party_9LC = "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `{}`,'''.format(
          c,e, c)
      for a, b in zip(year_statement_list, statement_list):
        select_table_4_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Third_Party_9LC = "{}" 
                                  THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(a, mySQL_column, b,e, a, b)
      ...
      third_party_table_4 = mycursor.fetchall()
    ...
    if complete_data_list[period_index][0] != period:
      ...
      for a, b in zip(complete_data_list[period_index][3], complete_data_list[period_index][2]):
        select_table_4_2 += '''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Third_Party_9LC = "{}" 
                                        THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(a, complete_data_list[period_index][1], b,
                                                                                                    e, a, b)
      select_table_4_3 = '''sum( CASE WHEN Year_Statement_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END)
                                          AS `Total` FROM Master GROUP BY Normalized_Income_Type_9LC ORDER BY `Total` DESC'''.format(
        cut_off)
      select_table_4 = select_table_4_1 + select_table_4_2 + select_table_4_3
      mycursor.execute(select_table_4)
      initial_third_party_table_4 = mycursor.fetchall()
      third_party_table_4 = []
      third_party_table_line = []
      for a in initial_third_party_table_4:
        third_party_table_line.append(a[0])
        for d in a[1: len(summary_years) + 1]:
          third_party_table_line.append(d)
        for b in a[1 + len(summary_years):len(a) - 1]:
          for c in range(split):
            third_party_table_line.append(b / split)
        third_party_table_line.append(a[-1])
        third_party_table_4.append(third_party_table_line)
        third_party_table_line = []
    list_sub_tables.append(third_party_table_4)

  table_4_element = []
  sub_total = 0
  all_columns = summary_years + statement_list
  for v in range(income_type_count):
    table_4_element.append(third_party_table_4[v][0])
    for u in range(1, len(all_columns) + 1):
      for w in list_sub_tables:
        sub_total += w[v][u]
      table_4_element.append(sub_total)
      sub_total = 0
    table_4_element.append(third_party_table_4[v][-1])
    table_4.append(table_4_element)
    table_4_element = []

  #Total per period
  total_list = ['Total']
  all_columns = statement_list + summary_years
  for i in range(1,len(all_columns)+2):
    period_total = 0
    for j in table_4:
      if j[i] == None:
        period_total += 0
  ...
  master_statement_list.append(statement_list)
  master_year_statement_list_element = (year_statement_list, divisions)
  master_year_statement_list.append(master_year_statement_list_element)
  ...
  if first_year_complete:
    for d, e in zip(range(len(summary_years)+1, (len(title_names_4) - minus_variable) * divisions + 2, divisions),
  ...
  if first_year_complete:
    ...
    for k in range(len(summary_years)+1, (len(title_names_4) - minus_variable) * divisions + 2, divisions):
    ...
    for q in range(len(summary_years)+1, (len(title_names_4)-minus_variable)*divisions+2, divisions):
...
#Sheet 4.1: Revenue Over Time By Third Party
  ...
  for s in third_party_list:
    ...
    if first_year_complete:
      for d, e in zip(range(len(summary_years)+1, (len(title_names_4_1) - minus_variable) * divisions + 2, divisions),
    ...
    if first_year_complete:
      ...
      for k in range(len(summary_years)+1, (len(title_names_4_1) - minus_variable) * divisions + 1, divisions):
  ...
  print(master_statement_list)
#Sheet 4.2: Revenue Over Time Data
  if first_year_complete:
    for z,a,b,c in zip(range(1, len(third_party_list)*5, 3), master_total_list, master_statement_list, master_year_statement_list):
      for x, y in zip(range(1,len(b)+3,c[1]),list(dict.fromkeys(c[0]))):
        ws_4_2.cell(row=x, column=z).value=y
      for x, y in zip(range(1,len(b)+len(summary_years)+3),b):
        ws_4_2.cell(row=x, column=z+1).value=y
      for x, y in zip(range(1,len(b)+len(summary_years)+3),a[len(summary_years)+1:-1]):
        ws_4_2.cell(row=x, column=z+2).value=y

  else:
    for z,a,b,c in zip(range(1, len(third_party_list)*4, 3), master_total_list, master_statement_list, master_year_statement_list):
      ws_4_2.cell(row=1, column=z).value = list(dict.fromkeys(c[0]))[0]
      for x, y in zip(range(len(detailed_remainder)+1,len(b)-len(detailed_remainder)+4,c[1]),list(dict.fromkeys(c[0]))[1:]):
        ws_4_2.cell(row=x, column=z).value=y
      for v, w in zip(range(1, len(detailed_remainder)+1),b[:len(detailed_remainder)]):
        ws_4_2.cell(row=v, column=z+1).value = w
      for x, y in zip(range(len(detailed_remainder)+1,len(b)-len(detailed_remainder)+4),b[len(detailed_remainder):]):
        ws_4_2.cell(row=x, column=z+1).value=y
      for v, w in zip(range(1, len(detailed_remainder)+1),a[1:2+len(detailed_remainder)]):
        ws_4_2.cell(row=v, column=z+2).value = w
      for x, y in zip(range(len(detailed_remainder)+1,len(b)-len(detailed_remainder)+4),a[1+len(detailed_remainder):]):
        ws_4_2.cell(row=x, column=z+2).value=y
  ...
  if first_year_complete:
    ...
    categories = Reference(ws_4_2, min_col=1, max_col=2, min_row=1,
                           max_row=len(statement_period_list) - len(summary_years)+graph_add)
    data = Reference(ws_4_2, min_col=3, max_col=3, min_row=1,
                     max_row=len(statement_period_list) - len(summary_years)+graph_add)
  else:
    chart = LineChart()
    categories = Reference(ws_4_2, min_col=1, max_col=2, min_row=1,
                           max_row=len(statement_period_list) - len(detailed_remainder) + graph_add)
    data = Reference(ws_4_2, min_col=3, max_col=3, min_row=1,
                     max_row=len(statement_period_list) - len(detailed_remainder) + graph_add)
  chart.add_data(data)
  ...
  graph_no = 2
  for a,b,c in zip(third_party_list, range(4, len(third_party_list)*5, 3),master_statement_list[1:]):
    chart = LineChart()
    categories = Reference(ws_4_2, min_col=b, max_col=b+1, min_row=1, max_row=len(c))
    data = Reference(ws_4_2, min_col=b+2, max_col=b+2, min_row=1, max_row=len(c))
    chart.add_data(data)
    chart.set_categories(categories)
    chart.y_axis.numFmt = '"$"#,##0.00'
...
#songvest('Edward Henson_61c49dfac94b68a289900e87')
...
#songvest2('Havoc_61c0ae68c94b68a289900e85')

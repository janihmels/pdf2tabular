...
def songxsourcexplaysxrevenue(database, df, filename):
...
#Build sheet
  ...
  column_names_1 = ['Song Title', 'Source', 'Plays', 'Revenue', 'Revenue Per Play']
  ...
  for s in all_songs:
    ...
    select_table_1 = '''SELECT Song_Name_9LC, Normalized_Source_9LC, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Plays_SB ELSE "" END) 
    ...
    AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Normalized_Source_9LC ORDER BY `Revenue` DESC'''.format(cut_off, cut_off, s_string)

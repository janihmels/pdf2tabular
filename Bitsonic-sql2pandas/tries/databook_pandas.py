...
from database_to_df import sql2pandas
import pandasql
from pandasql import sqldf
import pandas as pd


def databook_pandas(database, df, filename):
    ...
    pysqldf = lambda q: sqldf(q, {'Master': df})
    ...
    statement_period_half_list = list(pysqldf(q=find_period)['Statement_Period_Half_9LC'])
    ...
    total_list = list(pysqldf(q=find_total)['Total'])

    # df.groupby('Normalized_Income_Type_9LC').agg(`total` = (' case when statement_period_half_9lc >= '' then adjusted_royalty_sb else null end', 'sum'))
    ...
    total_table = pysqldf(q=find_total).values.tolist()
    ...
    source_table = pysqldf(q=find_source_table).values.tolist()
    ...
    income_table = pysqldf(q=find_income_table).values.tolist()
    ...
    song_table = pysqldf(q=find_song_table).values.tolist()
    ...
    initial_artist_table = pysqldf(q=find_artist_table).values.tolist()
    ...
    us_table = pysqldf(q=find_us_table).values.tolist()
    ...
    non_us_table = pysqldf(q=find_non_us_table).values.tolist()
...
if __name__ == "__main__":
    dbname = 'Adman Khan_61b775f7c94b68a289900e81'
    akkdjd = sql2pandas(dbname)

    databook_pandas(dbname, akkdjd, None)

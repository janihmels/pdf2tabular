...
def sourcexrevxhalf_sql(database, filename):
...
def sourcexrevxhalf(df, filename):
    third_parties = list(df['Third_Party_9LC'].drop_duplicates())
    rights_types = list(df['Rights_Type_9LC'].drop_duplicates())
    rights_types = [r for r in rights_types if r != '']  # deleting '' from the rights type list

    if len(rights_types) != 0:
        mojo = True
    else:
        mojo = False
    ...
    recent_year = sorted(list(df['Year_Statement_9LC'].drop_duplicates()))[-1]
    cut_off_year = current_year if recent_year == current_year else current_year - 1

    period = list(filter(lambda s: s != '' and int(s.split()[0]) <= current_year,
                         list(df['Statement_Period_Half_9LC'].drop_duplicates())))

    if len(period) > 10:
        statement_period_half = period[-10:]
        standart_cut_off = period[-10]
    else:
        statement_period_half = period
        standart_cut_off = period[0]

    year_cut_off = standart_cut_off[:4]
    year_periods = list(df['Year_Statement_9LC'].drop_duplicates())
    year_periods = list(dict.fromkeys([y for y in year_periods if y >= year_cut_off]))

    half_periods = df[['Year_Statement_9LC', 'Half_Statement_9LC']][
                        (df['Half_Statement_9LC'] != "") & (df['Statement_Period_Half_9LC'] >= standart_cut_off)].drop_duplicates()

    statement_period_half_list = list(half_periods['Year_Statement_9LC'] + ' ' + half_periods['Half_Statement_9LC'])
    year_statement_list_half = df['Year_Statement_9LC']
    half_statement_list = df['Half_Statement_9LC']
    year_list = list(dict.fromkeys(year_statement_list_half))
    ...
    quarter_periods = df[['Year_Statement_9LC', 'Quarter_Statement_9LC']][
        (df['Quarter_Statement_9LC'] != "") & (df['Statement_Period_Half_9LC'] >= standart_cut_off)].drop_duplicates()

    statement_period_quarter_list = list(half_periods['Year_Statement_9LC'] + ' ' + half_periods['Quarter_Statement_9LC'])
    year_statement_list_half = df['Year_Statement_9LC']
    quarter_statement_list = df['Quarter_Statement_9LC']
    year_list = list(dict.fromkeys(year_statement_list_half))
...
if __name__ == "__main__":
    ...
    # print(list(df['Rights_Type_9LC'].drop_duplicates()))

    sourcexrevxhalf(df, None)
    ...
    year_cut_off = int(standart_cut_off[:4])

    year_periods = list(df['Year_Statement_9LC'].drop_duplicates())

    year_periods = [y for y in year_periods if int(y) >= year_cut_off]


    year_periods = list(df['Year_Statement_9LC'].drop_duplicates())
    year_periods = list(dict.fromkeys([y for y in year_periods if int(y) >= year_cut_off]))

    print("-----------------------")
    # print(df.groupby(['Year_Statement_9LC', 'Quarter_Statement_9LC'][0])[['Year_Statement_9LC', 'Half_Statement_9LC']])
    # print(df.columns)
    # half_period = [a[['Year_Statement_9LC', 'Half_Statement_9LC']][a['Half_Statement_9LC'] >= standart_cut_off]
    #                for a in df.groupby(['Year_Statement_9LC', 'Quarter_Statement_9LC'])]
    #
    # print(half_period)

    # res = df[(df['Half_Statement_9LC'] != "") & (df['Statement_Period_Half_9LC'] >= '2019')].groupby(
    #     ['Year_Statement_9LC', 'Half_Statement_9LC']).agg({})

    print(standart_cut_off)
    # half_period = list(dict.fromkeys([(x['Year_Statement_9LC'], x['Half_Statement_9LC']) for _, x in df[['Year_Statement_9LC', 'Half_Statement_9LC']].iterrows() if
    #                                   x['Half_Statement_9LC'] != '' & x['Year_Statement_9LC'] >= standart_cut_off]))


    print(df['Statement_Period_Half_9LC'])

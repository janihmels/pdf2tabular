...
def temporegionxrevxhalf(database, df, filename):
...
#Find number of territories
  find_territories = '''SELECT Region_DD
                  FROM Master GROUP BY Region_DD '''
...
#Build sheet
  ...
  #Build table
  select_table_1_1 = '''SELECT Region_DD,'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                   ...
                   FROM Master
                   GROUP BY Region_DD ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  ...
  #Add total row
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters[3:]):

...
def summarynetgross(database, df, filename):
...
#Build sheet
  ...
  select_table_1_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}"
                   ...
                   GROUP BY Catalogue_9LC ORDER BY `Total` DESC'''.format(cut_off)
  ...
  for a, b, c, d in zip(range(1,len(column_names_1)+1),table_1[0], table_2[0], column_names_1):
    ...
    ws.cell(row=2, column=1).value = 'Net Revenue'
    ws.cell(row=3, column=1).value = 'Gross Revenue'
  ...
  for e in range(2, 4):
    ...
    ws.cell(row=e, column=len(column_names_1)).style = 'total_style'

  #Add net/gross % row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) + 1), column_letters_2):
    ws.cell(row=4, column=l).value = "={}2/{}3".format(m, m)
    ws.cell(row=4, column=l).number_format = '0.00%'
    ws.cell(row=4, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=4, column=l).font = Font(bold=True)
  ws.cell(row=4, column=1).value = 'Net/Gross %'
  ws.cell(row=4, column=1).font = Font(bold='True')
  ws.cell(row=4, column=1).fill = PatternFill("solid", fgColor="A6ACAF")

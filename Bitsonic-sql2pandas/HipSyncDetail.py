...
def hipsyncdetail(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Income II,Product No.'
  ws_1 = wb.create_sheet(title='Income II, Source, Product No.')
  ws_2 = wb.create_sheet(title='Product No. Coverage By Lines')
  ws_3 = wb.create_sheet(title='Product No. Coverage By Rev')
  ws_4 = wb.create_sheet(title='Income II')
  ws_5 = wb.create_sheet(title='Song,Income,Source,Product No.')

#Sheet 1: Income x Rev x Half
  #Find number of income types
  find_income_type = '''SELECT Normalized_Income_Type_9LC,Normalized_Income_Type_II_9LC, Source_Product_Number_SB
                        FROM Master WHERE Normalized_Income_Type_9LC = "Sync"
                        GROUP BY Normalized_Income_Type_9LC,Normalized_Income_Type_II_9LC, Source_Product_Number_SB'''
  mycursor.execute(find_income_type)
  income_type = [i[0] for i in mycursor.fetchall()]
  income_type_count = len(income_type)

  #Build table
  select_table_1_1 = '''SELECT Normalized_Income_Type_9LC,Normalized_Income_Type_II_9LC, Source_Product_Number_SB,'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                     AS `Total`
                     FROM Master WHERE Normalized_Income_Type_9LC = "Sync"
                     GROUP BY Normalized_Income_Type_9LC,Normalized_Income_Type_II_9LC, Source_Product_Number_SB ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  ...
  #Column names
  column_names_1 = ['Normalized Income Type','Normalized Income Type II', 'Source Product Number']
  ...
  table_rows = ws[2:income_type_count + 1]
  for j, k in zip(range(2, income_type_count + 2), final_table_1):
  ...
  for income_no in range(2, income_type_count + 2):
    ...
    ws.cell(row=income_no, column=2).alignment = Alignment(horizontal="left", vertical="center")
  ...
  for (l, m) in zip(range(4, len(column_names_1) - 1), column_letters[3:]):
    ws.cell(row=income_type_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, income_type_count + 1)
    ws.cell(row=income_type_count + 2, column=l).style = 'Comma'
    ws.cell(row=income_type_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=income_type_count + 2, column=l).font = Font(bold=True)
  ws.cell(row=income_type_count + 2, column=1).value = 'Total'
  ws.cell(row=income_type_count + 2, column=1).font = Font(bold='True')
  ws.cell(row=income_type_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws.cell(row=income_type_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")
...
#Sheet 2: Sync, Income Type II, Source
  #Find number of income types
  find_income_type = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC, Source_Product_Number_SB
                        FROM Master WHERE Normalized_Income_Type_9LC = "Sync" 
                        GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC, Source_Product_Number_SB'''
  mycursor.execute(find_income_type)
  income_type = [i[0] for i in mycursor.fetchall()]
  income_type_count = len(income_type)

  #Build table
  select_table_1_1 = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC, Source_Product_Number_SB,'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}"
                             THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                     AS `Total`
                     FROM Master WHERE Normalized_Income_Type_9LC = "Sync"
                     GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Normalized_Source_9LC, Source_Product_Number_SB ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  final_table_1 = mycursor.fetchall()

  #Column names
  column_names_1 = ['Normalized Income Type','Normalized Income Type II','Normalized Source', 'Source Product Number']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
  ...
  for income_no in range(2, income_type_count + 2):
    ...
    ws_1.cell(row=income_no, column=2).alignment = Alignment(horizontal="left", vertical="center")
    ws_1.cell(row=income_no, column=3).alignment = Alignment(horizontal="left", vertical="center")
  ...
  for (l, m) in zip(range(5, len(column_names_1) - 1), column_letters[4:]):
    ...
    ws_1.cell(row=income_type_count + 2, column=l).style = 'Comma'
    ws_1.cell(row=income_type_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_1.cell(row=income_type_count + 2, column=l).font = Font(bold=True)
...
#Sheet 3: % Song Product Number By Lines
#Build table
  select_table_1_1 = '''SELECT'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (
      ''' COUNT( IF( (Source_Product_Number_SB = "") AND Year_Statement_9LC = "{}" AND {} = "{}",1,NULL)) `{} {} Null Lines`,'''.format(
        j, mySQL_column, k, j, k))
  select_table_1_3 = '''COUNT( IF ((Source_Product_Number_SB = "") AND {} >= "{}",1,NULL)) `Total Null Lines`
                   FROM Master WHERE {} <> "" AND Normalized_Income_Type_9LC = "Sync"'''.format(cut_off_field, cut_off, cut_off_field)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  ...
  for j, k in zip(year_statement_list, statement_list):
    select_table_2_2 += (
      ''' COUNT( IF( (Source_Product_Number_SB <> "") AND Year_Statement_9LC = "{}" AND {} = "{}",1,NULL)) `{} {} Non Null Lines`,'''.format(
        j, mySQL_column, k, j, k))
  select_table_2_3 = '''COUNT( IF ( (Source_Product_Number_SB <> "") AND {} >= "{}",1,NULL)) `Total Non Null Lines`
                   FROM Master WHERE {} <> "" AND Normalized_Income_Type_9LC = "Sync"'''.format(cut_off_field, cut_off, cut_off_field)
  ...
  for a, b, c, d in zip(range(2, len(column_names_1) + 2), table_1[0], table_2[0], column_names_1):
    ws_2.cell(row=2, column=a).value = b
    ws_2.cell(row=3, column=a).value = c
    ws_2.cell(row=2, column=1).value = 'No. Null Lines'
    ws_2.cell(row=3, column=1).value = 'No. Non-Null Lines'
    ws_2.cell(row=2, column=a).number_format = '#,##0'
    ws_2.cell(row=3, column=a).number_format = '#,##0'
  for e in range(2, 4):
    ws_2.cell(row=e, column=1).style = 'name_style'
    ws_2.cell(row=e, column=len(column_names_1)).font = Font(bold=True)

  # Total row
  for f, g in zip(range(2, len(column_names_1) + 1), column_letters[1:]):
    ws_2.cell(row=4, column=f).value = '=SUM({}2:{}3)'.format(g, g)
    ws_2.cell(row=4, column=f).number_format = '#,##0'
    ws_2.cell(row=4, column=f).font = Font(bold=True)
  ws_2.cell(row=4, column=1).value = 'Total'
  ws_2.cell(row=4, column=1).style = 'title_style'

  # Percentage table
  for h, i in zip(range(2, len(column_names_1) + 1), column_letters[1:]):
    ws_2.cell(row=7, column=h).value = '={}2/{}4'.format(i, i)
    ws_2.cell(row=7, column=h).number_format = '0.00%'
    ws_2.cell(row=8, column=h).value = '={}3/{}4'.format(i, i)
    ws_2.cell(row=8, column=h).number_format = '0.00%'
  ws_2.cell(row=7, column=1).value = '% Null Lines'
  ws_2.cell(row=8, column=1).value = '% Non-Null Lines'

  # Add column names to percentage table
  for j, k in zip(range(1, len(column_names_1) + 1), column_names_1):
    ws_2.cell(row=6, column=j).value = k
    ws_2.cell(row=6, column=j).style = 'title_style'
  ...
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Source_Product_Number_SB = "" AND Year_Statement_9LC = "{}" AND {} = "{}"
                                 THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j,
                                                                                             k))
  select_table_1_3 = '''sum( CASE WHEN Source_Product_Number_SB = "" AND {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                       AS `Total`
                       FROM Master WHERE Normalized_Income_Type_9LC = "Sync"'''.format(
  ...
  select_table_2_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_2_2 += (''' sum( CASE WHEN Source_Product_Number_SB <> "" AND Year_Statement_9LC = "{}" AND {} = "{}"
                                     THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k,
                                                                                                 j,
                                                                                                 k))
  select_table_2_3 = '''sum( CASE WHEN Source_Product_Number_SB <> "" AND {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) 
                         AS `Total`
                         FROM Master WHERE Normalized_Income_Type_9LC = "Sync"'''.format(
    cut_off_field, cut_off)
  select_table_2 = select_table_2_1 + select_table_2_2 + select_table_2_3
  ...
  # Add table to worksheet
  for a, b, c, d in zip(range(2, len(column_names_1) + 2), table_1[0], table_2[0], column_names_1):
    ws_3.cell(row=2, column=a).value = b
    ws_3.cell(row=3, column=a).value = c
    ws_3.cell(row=2, column=1).value = 'Null Revenue'
    ws_3.cell(row=3, column=1).value = 'Non-Null Revenue'
    ws_3.cell(row=2, column=a).number_format = '#,##0.00'
    ws_3.cell(row=3, column=a).number_format = '#,##0.00'
  for e in range(2, 4):
    ws_3.cell(row=e, column=1).style = 'name_style'
    ws_3.cell(row=e, column=len(column_names_1)).font = Font(bold=True)

  # Total row
  for f, g in zip(range(2, len(column_names_1) + 1), column_letters[1:]):
    ws_3.cell(row=4, column=f).value = '=SUM({}2:{}3)'.format(g, g)
    ws_3.cell(row=4, column=f).number_format = '#,##0.00'
    ws_3.cell(row=4, column=f).font = Font(bold=True)
  ws_3.cell(row=4, column=1).value = 'Total'
  ws_3.cell(row=4, column=1).style = 'title_style'

  # Percentage table
  for h, i in zip(range(2, len(column_names_1) + 1), column_letters[1:]):
    ws_3.cell(row=7, column=h).value = '={}2/{}4'.format(i, i)
    ws_3.cell(row=7, column=h).number_format = '0.00%'
    ws_3.cell(row=8, column=h).value = '={}3/{}4'.format(i, i)
    ws_3.cell(row=8, column=h).number_format = '0.00%'
  ws_3.cell(row=7, column=1).value = '% Null Revenue'
  ws_3.cell(row=8, column=1).value = '% Non-Null Revenue'

  # Add column names to percentage table
  for j, k in zip(range(1, len(column_names_1) + 1), column_names_1):
    ws_3.cell(row=6, column=j).value = k
    ws_3.cell(row=6, column=j).style = 'title_style'

#Sheet 5: Sync, Income II x Rev x Half
#Find number of sources
  find_source_type = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC
                        FROM Master WHERE Normalized_Income_Type_9LC = "Sync" GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC'''
  mycursor.execute(find_source_type)
  sources = [i[0] for i in mycursor.fetchall()]
  source_count = len(sources)

  # Build table
  select_table_1_1 = '''SELECT Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" 
                               THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                       ...
                       FROM Master WHERE Normalized_Income_Type_9LC = "Sync"
                       GROUP BY Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC ORDER BY `Total` DESC'''.format(
    cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  ...
  # Column names
  column_names_1 = ['Normalized Income Type', 'Normalized Income Type II']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
  ...
  # Size of worksheet
  for column_no in range(1, len(column_names_1) + 1):
    for row_no in range(1, source_count + 1):
  ...
  # Add table to worksheet
  table_rows = ws_4[2:source_count + 1]
  for j, k in zip(range(2, source_count + 2), final_table_1):
    for l, m in zip(range(1, len(column_names_1)), range(len(k))):
      ws_4.cell(row=j, column=l).value = k[m]
      ws_4.cell(row=j, column=l).style = 'number_style'

  for income_no in range(2, source_count + 2):
    ws_4.cell(row=income_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws_4.cell(row=income_no, column=2).alignment = Alignment(horizontal="left", vertical="center")
  ...
  for (l, m) in zip(range(3, len(column_names_1) - 1), column_letters[2:]):
    ws_4.cell(row=source_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_count + 1)
    ws_4.cell(row=source_count + 2, column=l).style = 'Comma'
    ws_4.cell(row=source_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_4.cell(row=source_count + 2, column=l).font = Font(bold=True)
  ws_4.cell(row=source_count + 2, column=1).value = 'Total'
  ws_4.cell(row=source_count + 2, column=1).font = Font(bold='True')
  ws_4.cell(row=source_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_4.cell(row=source_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

  # Add % of revenue column
  total_column_letter = column_letters[len(column_names_1) - 3]
  for n in range(2, source_count + 2):
  ...
  for o in range(3, source_count + 2):
...
#Sheet 5: Song,Income II,Source x Rev x Half
  # Find number of sources
  find_source_type = '''SELECT Song_Name_9LC,Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,Normalized_Source_9LC,Source_Product_Number_SB
                        FROM Master WHERE Normalized_Income_Type_9LC = "Sync" GROUP BY Song_Name_9LC,Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,Normalized_Source_9LC,Source_Product_Number_SB'''
  mycursor.execute(find_source_type)
  sources = [i[0] for i in mycursor.fetchall()]
  source_count = len(sources)

  #Build table
  select_table_1_1 = '''SELECT Song_Name_9LC,Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,Normalized_Source_9LC,Source_Product_Number_SB,'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" 
                               THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                       THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                       FROM Master WHERE Normalized_Income_Type_9LC = "Sync"
                       GROUP BY Song_Name_9LC,Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,Normalized_Source_9LC,Source_Product_Number_SB ORDER BY `Total` DESC'''.format(
    cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  ...
  # Column names
  column_names_1 = ['Song Title','Normalized Income Type', 'Normalized Income Type II','Normalized Source','Source Product Number']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
  ...
  # Add table to worksheet
  table_rows = ws_5[2:source_count + 1]
  for j, k in zip(range(2, source_count + 2), final_table_1):
    for l, m in zip(range(1, len(column_names_1)), range(len(k))):
  ...
  for income_no in range(2, source_count + 2):
    ...
    ws_5.cell(row=income_no, column=4).alignment = Alignment(horizontal="left", vertical="center")
  ...
  for (l, m) in zip(range(6, len(column_names_1) - 1), column_letters[5:]):
    ws_5.cell(row=source_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_count + 1)
    ws_5.cell(row=source_count + 2, column=l).style = 'Comma'
    ws_5.cell(row=source_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_5.cell(row=source_count + 2, column=l).font = Font(bold=True)
  ws_5.cell(row=source_count + 2, column=1).value = 'Total'
  ws_5.cell(row=source_count + 2, column=1).font = Font(bold='True')
  ws_5.cell(row=source_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_5.cell(row=source_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

...
def territoryxrevxhalf(database, df, filename):
...
#Build main pivot table
  ...
  for s in third_party_type_list:
    ...
    select_table_1_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}"
                     ...
                     AND Rights_Type_9LC = "{}" AND Country_SB IN ("United States", "United States of America", "US", "USA", "USA AND DOMINIONS")
    ...
    select_table_2_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}"
                           THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                           FROM Master WHERE Statement_Period_Half_9LC <> ""
                           AND Rights_Type_9LC = "{}" AND Country_SB NOT IN ("United States", "United States of America", "US", "USA", "USA AND DOMINIONS") 
                           ORDER BY `Total` DESC'''.format(cut_off, s[0])
...
#Sheet 2: Combined sheet
  ...
  select_table_1_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}"
                   THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                   FROM Master WHERE Statement_Period_Half_9LC <> "" AND Country_SB IN ("United States", "United States of America", "US", "USA", "USA AND DOMINIONS") 
  ...
  select_table_2_3 = '''sum( CASE WHEN Statement_Period_Half_9LC >= "{}"
                             THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                             FROM Master WHERE Statement_Period_Half_9LC <> ""
                             AND Country_SB NOT IN ("United States", "United States of America", "US", "USA", "USA AND DOMINIONS") 
                             ORDER BY `Total` DESC'''.format(cut_off)
  select_table_2 = select_table_2_1 + select_table_2_2 + select_table_2_3
  mycursor.execute(select_table_2)

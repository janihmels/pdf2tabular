...
def songIDISWCISRCincomesongshare(database, df, filename):
...
#Find number of songs
  find_songs = '''SELECT Song_Name_9LC, Payer_Song_ID_SB, SONGID_ISWC_SB, SONGID_ISRC_SB,Normalized_Income_Type_II_9LC, Income_Type_SB, Payee_Song_Share_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Payer_Song_ID_SB, SONGID_ISWC_SB, SONGID_ISRC_SB,Normalized_Income_Type_II_9LC, Income_Type_SB,Payee_Song_Share_SB'''
...
#Build sheets
  ...
  #Worksheet title
  ws.title = "Song,ID,ISWC,ISRC,Income Type,Song Share".format(period_name)

  #Build table
  select_table_1 = '''SELECT Song_Name_9LC, Payer_Song_ID_SB,SONGID_ISWC_SB, SONGID_ISRC_SB, Normalized_Income_Type_II_9LC, Income_Type_SB, Payee_Song_Share_SB FROM Master WHERE Song_Name_9LC <> "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Payer_Song_ID_SB,SONGID_ISWC_SB, SONGID_ISRC_SB,  Normalized_Income_Type_II_9LC,Income_Type_SB,  Payee_Song_Share_SB'''
  ...
  pool_rev_1 = '''SELECT Song_Name_9LC, Payer_Song_ID_SB, SONGID_ISWC_SB, SONGID_ISRC_SB,Normalized_Income_Type_II_9LC, Income_Type_SB, Payee_Song_Share_SB FROM Master WHERE Song_Name_9LC = "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Payer_Song_ID_SB,SONGID_ISWC_SB, SONGID_ISRC_SB,  Normalized_Income_Type_II_9LC,Income_Type_SB, Payee_Song_Share_SB'''
  ...
  column_names_1 = ['Song Title', 'Payer ID','ISWC','ISRC',  'Normalized Income Type II','Income Type', 'Payee Song Share']
  ...
  for song_no in range(2, song_count + 2):
    ...
    ws.cell(row=song_no, column=2).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=song_no, column=3).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=song_no, column=4).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=song_no, column=len(column_names_1)).alignment = Alignment(horizontal="right", vertical="center")

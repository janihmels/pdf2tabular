...
def hipnetgross(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Net v Gross Total By Song'
  ws_1 = wb.create_sheet(title='Net v Gross Summary')

#Sheet 1: Net/Gross Total By Song
  #Find number of songs
  find_songs = '''SELECT Song_Name_9LC FROM Master GROUP BY Song_Name_9LC'''
  mycursor.execute(find_songs)
  songs = [i[0] for i in mycursor.fetchall()]
  song_count = len(songs)

  #Build table
  select_table_1 = '''SELECT Song_Name_9LC, sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Net Total`,
                      sum( CASE WHEN {} >= "{}" THEN Payee_Gross_Royalty_Amount_SB ELSE "" END) AS `Gross Total` FROM Master WHERE Song_Name_9LC <> "Pool Revenue"
                      GROUP BY Song_Name_9LC ORDER BY `Net Total` DESC'''.format(cut_off_field,cut_off,cut_off_field,
                                                                                                                cut_off)
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()

  #Add pool revenue line at bottom
  pool_rev_1 = '''SELECT Song_Name_9LC, sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Net Total`,
                      sum( CASE WHEN {} >= "{}" THEN Payee_Gross_Royalty_Amount_SB ELSE "" END) AS `Gross Total` FROM Master WHERE Song_Name_9LC = "Pool Revenue"
                      GROUP BY Song_Name_9LC ORDER BY `Net Total` DESC'''.format(cut_off_field,cut_off,cut_off_field,
                                                                                                                cut_off)
  ...
  column_names_1 = ['Song Title', 'Net Total', 'Gross Total']
  ...
  for j, k in zip(range(2, song_count + 2), final_table_1):
    for l, m in zip(range(1, len(column_names_1)+1), range(len(k))):
  ...
  for n in range(2, song_count+2):
    ws.cell(row=n, column=4).value = '=IFERROR(B{}/C{},"N/A")'.format(n,n)
    ws.cell(row=n, column=4).number_format = '0.00%'
  ws.cell(row=1, column=4).value = 'Net/Gross %'
  ws.cell(row=1, column=4).style = 'title_style'
...
#Sheet 2: Net/Gross Summary
  #Build table
  select_table_1_1 = '''SELECT Catalogue_9LC,'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                   ...
                   FROM Master
                   GROUP BY Catalogue_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()

  select_table_2_1 = '''SELECT Catalogue_9LC,'''
  select_table_2_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_2_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}"
                             THEN Payee_Gross_Royalty_Amount_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_2_3 = '''sum( CASE WHEN {} >= "{}"
                     THEN Payee_Gross_Royalty_Amount_SB ELSE "" END) AS `Total`
                     FROM Master
                     GROUP BY Catalogue_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off)
  ...
  #Add table to worksheet
  for a, b, c, d in zip(range(1,len(column_names_1)+1),table_1[0], table_2[0], column_names_1):
    ws_1.cell(row=2, column=a).value = b
    ws_1.cell(row=3, column=a).value = c
    ws_1.cell(row=2, column=1).value = 'Net Revenue'
    ws_1.cell(row=3, column=1).value = 'Gross Revenue'
    ws_1.cell(row=2, column=a).style = 'number_style'
    ws_1.cell(row=3, column=a).style = 'number_style'
  for e in range(2, 4):
    ws_1.cell(row=e, column=1).style = 'name_style'
    ws_1.cell(row=e, column=len(column_names_1)).style = 'total_style'

  #Add net/gross % row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) + 1), column_letters_2):
    ws_1.cell(row=4, column=l).value = "={}2/{}3".format(m, m)
    ws_1.cell(row=4, column=l).number_format = '0.00%'
    ws_1.cell(row=4, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_1.cell(row=4, column=l).font = Font(bold=True)
  ws_1.cell(row=4, column=1).value = 'Net/Gross %'
  ws_1.cell(row=4, column=1).font = Font(bold='True')
  ws_1.cell(row=4, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_1.cell(row=4, column=1).alignment = Alignment(horizontal="center", vertical="center")

...
def hipterritories(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Country'
  ws_1 = wb.create_sheet(title='Regional Block')
  ws_2 = wb.create_sheet(title='Regional Block,Region,Country')

#Sheet 1: Country x Rev x Half
  #Find number of countries
  find_countries = '''SELECT Normalized_Country_9LC FROM Master GROUP BY Normalized_Country_9LC'''
  mycursor.execute(find_countries)
  countries = [i[0] for i in mycursor.fetchall()]
  country_count = len(countries)

  #Build table
  select_table_1_1 = '''SELECT Normalized_Country_9LC,'''
  select_table_1_2 = ""
  ...
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                     ...
                     FROM Master
                     GROUP BY Normalized_Country_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off)
  ...
  for column_no in range(1, len(column_names_1) + 1):
    for row_no in range(1, country_count + 1):
  ...
  table_rows = ws[2:country_count + 1]
  ...
  for country_no in range(2, country_count + 2):
    ws.cell(row=country_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws.cell(row=country_no, column=(len(column_names_1) - 2)).font = Font(bold=True)

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters_2):
    ws.cell(row=country_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, country_count + 1)
    ws.cell(row=country_count + 2, column=l).style = 'Comma'
    ws.cell(row=country_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=country_count + 2, column=l).font = Font(bold=True)
  ws.cell(row=country_count + 2, column=1).value = 'Total'
  ws.cell(row=country_count + 2, column=1).font = Font(bold='True')
  ws.cell(row=country_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws.cell(row=country_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

  #Add % of revenue column
  total_column_letter = column_letters[len(column_names_1) - 3]
  for n in range(2, country_count + 2):
    ws.cell(row=n, column=len(column_names_1) - 1).value = "=({}{}/{}{})".format(total_column_letter, n,
                                                                                 total_column_letter, country_count + 2)
  ...
  for o in range(3, country_count + 2):
...
#Sheet 2: Regional Block x Rev x Half
  #Find number of countries
  find_countries = '''SELECT Regional_Block_DD FROM Master GROUP BY Regional_Block_DD'''
  mycursor.execute(find_countries)
  countries = [i[0] for i in mycursor.fetchall()]
  country_count = len(countries)

  #Build table
  select_table_1_1 = '''SELECT Regional_Block_DD,'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}"
                             THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                     THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                     FROM Master
                     GROUP BY Regional_Block_DD ORDER BY `Total` DESC'''.format(cut_off_field,cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  final_table_1 = table_1
  ...
  #Add table to worksheet
  table_rows = ws_1[2:country_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      cell.value = j[k]
      cell.style = 'Comma'
      ...
      cell.font = Font(name="Calibri", size="11")
  for country_no in range(2, country_count + 2):
    ws_1.cell(row=country_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws_1.cell(row=country_no, column=(len(column_names_1) - 2)).font = Font(bold=True)

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters_2):
    ws_1.cell(row=country_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, country_count + 1)
    ws_1.cell(row=country_count + 2, column=l).style = 'Comma'
    ws_1.cell(row=country_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_1.cell(row=country_count + 2, column=l).font = Font(bold=True)
  ws_1.cell(row=country_count + 2, column=1).value = 'Total'
  ws_1.cell(row=country_count + 2, column=1).font = Font(bold='True')
  ws_1.cell(row=country_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_1.cell(row=country_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")
...
#Sheet 3: Regional Block,Region,Country x Rev x Half
  #Find number of countries
  find_countries = '''SELECT Regional_Block_DD, Region_DD, Normalized_Country_9LC FROM Master GROUP BY Regional_Block_DD, Region_DD, Normalized_Country_9LC'''
  mycursor.execute(find_countries)
  countries = [i[0] for i in mycursor.fetchall()]
  country_count = len(countries)

  #Build table
  select_table_1_1 = '''SELECT Regional_Block_DD, Region_DD, Normalized_Country_9LC,'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}"
                             THEN Adjusted_Royalty_SB ELSE "" END) AS `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''sum( CASE WHEN {} >= "{}"
                     THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                     FROM Master
                     GROUP BY Regional_Block_DD, Region_DD, Normalized_Country_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  final_table_1 = table_1
  ...
  #Create list of column names
  column_names_1 = ['Regional Block','Region','Country']
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Revenue')
  ...
  #Add table to worksheet
  table_rows = ws_2[2:country_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      cell.value = j[k]
      cell.style = 'Comma'
      ...
      cell.font = Font(name="Calibri", size="11")
  for country_no in range(2, country_count + 2):
    ws_2.cell(row=country_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws_2.cell(row=country_no, column=2).alignment = Alignment(horizontal="left", vertical="center")
    ws_2.cell(row=country_no, column=3).alignment = Alignment(horizontal="left", vertical="center")
    ws_2.cell(row=country_no, column=(len(column_names_1) - 2)).font = Font(bold=True)

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(4, len(column_names_1) - 1), column_letters[3:]):
    ws_2.cell(row=country_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, country_count + 1)
    ws_2.cell(row=country_count + 2, column=l).style = 'Comma'
    ws_2.cell(row=country_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_2.cell(row=country_count + 2, column=l).font = Font(bold=True)
  ws_2.cell(row=country_count + 2, column=1).value = 'Total'
  ws_2.cell(row=country_count + 2, column=1).font = Font(bold='True')
  ws_2.cell(row=country_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_2.cell(row=country_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

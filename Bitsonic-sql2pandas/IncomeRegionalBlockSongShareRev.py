...
def incomeregionalblocksongsharerev(database, df, filename):
...
#Create sheets
  wb = Workbook()
  ws_1 = wb.active
  ws_1.title = 'Song, Regional Block, Income, Income II, Song Share'
...
#Find number of songs
  find_songs = '''SELECT Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,  Payee_Song_Share_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Song_Share_SB'''
...
#Build main pivot table
  ...
  #Build table
  select_table_1 = '''SELECT Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Song_Share_SB, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC <> "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC, Payee_Song_Share_SB ORDER BY `Total` DESC'''.format(cut_off)
  ...
  pool_rev_1 = '''SELECT Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,Payee_Song_Share_SB, sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master WHERE Song_Name_9LC = "Pool Revenue"
                      AND Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Regional_Block_SB, Normalized_Income_Type_9LC, Normalized_Income_Type_II_9LC,Payee_Song_Share_SB'''.format(cut_off)
  mycursor.execute(pool_rev_1)
  pool_revenue_1 = mycursor.fetchall()
  final_table_1 = table_1 + pool_revenue_1

  #Create list of column names
  column_names_1 = ['Song Name', 'Regional Block', 'Normalized Income Type', 'Normalized Income Type II','Payee Percent Received', 'Total Revenue']
  ...
  for song_no in range(2, song_count + 2):
    ...
    ws_1.cell(row=song_no, column=5).alignment = Alignment(horizontal="right", vertical="center")
    ws_1.cell(row=song_no, column=6).alignment = Alignment(horizontal="right", vertical="center")

  #Total row
  ws_1.cell(row=song_no + 1, column=1).value = 'Total'
  ws_1.cell(row=song_no + 1, column=1).style = 'total_label_style'
  ws_1.cell(row=song_no + 1, column=6).value = '=SUM(F2:F{})'.format(song_no)
  ws_1.cell(row=song_no + 1, column=6).style = 'total_style'

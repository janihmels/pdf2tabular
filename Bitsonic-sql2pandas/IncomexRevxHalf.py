...
def incomexrevxhalf(database, df, filename):
...
#Create sheets
  wb = Workbook()
  ws = wb.active
  #ws_1 = wb.create_sheet(title="S Income x Rev x Half", index=0)
  ws_1 = wb.create_sheet(title='S Income Type x Rev x Half', index=0)
...
#Create list of column names
  year_column_names = ['Normalized Income Type']
  ...
  half_column_names = ['Normalized Income Type']
  ...
  quarter_column_names = ['Normalized Income Type']
...
#Build main pivot table
  ws.title = "M Income x Rev x Half"
  total_row_no = -2
  ...
  #Add table to worksheet
    ...
    for (row, j) in zip(final_table_rows, table):

...
def songxplaysxrevenue(database, df, filename):
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Song x Plays x Rev"

  #Build table
  select_table_1 = '''SELECT Song_Name_9LC, sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Plays_SB ELSE "" END) AS `Plays`,
  sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Revenue` FROM Master 
  WHERE Song_Name_9LC <> "Pool Revenue" GROUP BY Song_Name_9LC ORDER BY `Revenue` desc '''.format(cut_off,cut_off)
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  select_table_2 = '''SELECT Song_Name_9LC, sum( CASE WHEN Song_Name_9LC = "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Plays_SB ELSE "" END) AS `Plays`,
    sum( CASE WHEN Song_Name_9LC = "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Revenue` FROM Master 
    WHERE Song_Name_9LC = "Pool Revenue" GROUP BY Song_Name_9LC '''.format(cut_off,cut_off)
  mycursor.execute(select_table_2)
  table_2 = mycursor.fetchall()
  final_table_1 = table_1 + table_2

  #Create list of column names
  column_names_1 = ['Song Title', 'Plays', 'Revenue','Revenue Per Play']
  ...
  ws.cell(row=song_count + 2, column=2).number_format = '#,##0'

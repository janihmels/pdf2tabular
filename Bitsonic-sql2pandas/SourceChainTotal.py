...
def sourcechaintotal(database, df, filename):
...
#Find number of source types
  find_source_type = '''SELECT TRIM(Source_Chain_SB)
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY TRIM(Source_Chain_SB)'''
...
#Build main pivot table
  ...
  #Worksheet title
  ws.title = "Source Chain Total"

  #Build table
  select_table_1 = '''SELECT TRIM(Source_Chain_SB), sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master 
  GROUP BY TRIM(Source_Chain_SB) ORDER BY `Total` desc '''.format(cut_off)
  ...
  for (l, m) in zip(range(2, len(column_names_1)+1), column_letters_2):
    ...
    ws.cell(row=source_count + 2, column=l).number_format = '#,##0.00'
  ...
  ws.cell(row=source_count + 2, column=2).number_format = '#,##0.00'

...
def songdateISWCxrevxhalf(database, df, filename):
...
#Find number of songs
  find_songs = '''SELECT Song_Name_9LC, Release_Date_9LC, SONGID_ISWC_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Release_Date_9LC, SONGID_ISWC_SB '''
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Song,Date,ISWC x Rev x {}".format(period_name)
  ...
  select_table_1_3 = '''sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}"
                   ...
                   GROUP BY Song_Name_9LC, Release_Date_9LC,SONGID_ISWC_SB ORDER BY `Total` DESC'''.format(cut_off)

...
def sourcechainxlinesxhalf(database, df, filename):
...
#Find number of songs
  find_songs = '''SELECT TRIM(Source_Chain_SB)
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Source Chain x Lines x {}".format(period_name)

  #Build table
  select_table_1_1 = '''SELECT TRIM(Source_Chain_SB),'''
  select_table_1_2 = ""
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (''' COUNT( IF(Year_Statement_9LC = "{}" AND {} = "{}",1,NULL)) `{} {}`,'''.format(j,mySQL_column,k, j,k))
  select_table_1_3 = '''COUNT( IF(Statement_Period_Half_9LC >= "{}",1,NULL)) `Total`
  ...
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters_2):
    ...
    ws.cell(row=song_count + 2, column=l).number_format = '#,##0'

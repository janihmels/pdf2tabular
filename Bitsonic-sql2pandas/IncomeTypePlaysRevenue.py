...
def incomexplaysxrevenue(database, df, filename):
...
#Find number of income types
  find_income_type = '''SELECT Normalized_Income_Type_9LC
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Normalized_Income_Type_9LC'''
  mycursor.execute(find_income_type)
  income_types = [i[0] for i in mycursor.fetchall()]
  income_count = len(income_types)
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Income Type x Plays x Rev"

  #Build table
  select_table_1 = '''SELECT Normalized_Income_Type_9LC, sum( CASE WHEN Normalized_Income_Type_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}" THEN Plays_SB ELSE "" END) AS `Plays`,
  sum( CASE WHEN Statement_Period_Half_9LC >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Revenue` FROM Master 
  GROUP BY Normalized_Income_Type_9LC ORDER BY `Revenue` desc '''.format(cut_off,cut_off)
  mycursor.execute(select_table_1)
  table_1 = mycursor.fetchall()
  final_table_1 = table_1

  #Create list of column names
  column_names_1 = ['Income Type', 'Plays', 'Revenue','Revenue Per Play']

  #Size of worksheet
  for column_no in range(1, len(column_names_1) + 1):
    for row_no in range(1, income_count + 1):
  ...
  table_rows = ws[2:income_count + 1]
  ...
  for song_no in range(2, income_count + 2):
  ...
  for (l, m) in zip(range(2, len(column_names_1)), column_letters_2):
    ws.cell(row=income_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, income_count + 1)
    ws.cell(row=income_count + 2, column=l).style = 'Comma'
    ws.cell(row=income_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=income_count + 2, column=l).font = Font(bold=True)
  ws.cell(row=income_count + 2, column=2).number_format = '#,##0'
  ws.cell(row=income_count + 2, column=1).value = 'Total'
  ws.cell(row=income_count + 2, column=1).font = Font(bold='True')
  ws.cell(row=income_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws.cell(row=income_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

  #Add revenue per play
  total_column_letter = column_letters[len(column_names_1) - 3]
  for n in range(2, income_count + 2):

...
def thirdpartyxsongxrevxhalf(database, df, filename):
  mycursor = pandas_cursor(df=df)

#Create sheets
  wb = Workbook()
  ws = wb.active
  ws.title = "Payor x Song x Rev x Half"
...
#Create list of column names
  half_column_names = ['Payor', 'Song Title']
  ...
  quarter_column_names = ['Payor', 'Song Title']
  ...
  year_column_names = ['Payor','Song Title']
...
#Build tables
  ...
  for r in third_party_type_list:
    for s in r[1:]:
      ...
      for j,k in zip(year_statement_list, statement_list):
        select_table_2 += (''' sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Song_Name_9LC <> "Pool Revenue"
      ...
      select_table_3 = '''sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" AND 
                          ...
                          GROUP BY Third_Party_9LC, Song_Name_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off,s)
      ...
      for l,m in zip(year_statement_list, statement_list):
        pool_rev_2 += ('''sum( CASE WHEN Year_Statement_9LC = "{}" AND {} = "{}" AND Song_Name_9LC = "Pool Revenue"
  ...
  #Add percentage and cumulative percentage columns
      ...
      ws.cell(row=header_row_no, column=total_column_no+1).value = 'Cumulative %'
...
#thirdpartyxsongxrevxhalf('DJ Battlecat_616c91a05f278b92afbaa5ae')
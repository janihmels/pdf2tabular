...
def songIDxrevxhalf(database, df, filename):
...
#Find number of songs
  find_songs = '''SELECT Song_Name_9LC, Payer_Song_ID_SB
                  FROM Master WHERE Statement_Period_Half_9LC <> "" GROUP BY Song_Name_9LC, Payer_Song_ID_SB '''
...
#Build sheet
  ...
  #Worksheet title
  ws.title = "Song,ID x Rev x {}".format(period_name)
  ...
  select_table_1_3 = '''sum( CASE WHEN Song_Name_9LC <> "Pool Revenue" AND Statement_Period_Half_9LC >= "{}"
                   ...
                   GROUP BY Song_Name_9LC, Payer_Song_ID_SB ORDER BY `Total` DESC'''.format(cut_off)

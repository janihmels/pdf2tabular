...
def songxincomexrevxhalf(database, df, filename):
...
#Create list of column names
  year_column_names = ['Song Title', 'Income Type']
  ...
  half_column_names = ['Song Title', 'Income Type']
  ...
  quarter_column_names = ['Song Title', 'Income Type']
...
#Build first table
  if mojo:
    ...
    if third_party_type_list[0][0] == "":
      ws.title = "Song x Income x Rev x Half"
    else:
      ws.title = "({}) Song x Income x Rev x Half".format(third_party_type_list[0][0])
    ...
    for s in all_songs:
      ...
      select_table_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                          ...
                          GROUP BY Song_Name_9LC, Normalized_Income_Type_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off,s_string, third_party_type_list[0][0])
  ...
  #Build rest of tables
    ...
    #Build song tables
      ...
      for s in all_songs:
        ...
        select_table_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                              FROM Master WHERE Song_Name_9LC = "{}" AND Rights_Type_9LC = "{}" 
                              GROUP BY Song_Name_9LC, Normalized_Income_Type_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off,
                                                                                                                 s_string,
                                                                                                                 i[0])
        select_table = select_table_1 + select_table_2 + select_table_3
        mycursor.execute(select_table)
...
#Create combined sheet
    ...
    ws_1 = wb.active
    ws_1.title = 'Song x Income x Rev x Half'
    ...
    #Create list of column names
    column_names_1 = ['Song Title', 'Income Type']
    ...
    for s in all_songs_combined:
      ...
      select_table_1_3 = '''sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total`
                            ...
                            GROUP BY Song_Name_9LC, Normalized_Income_Type_9LC ORDER BY `Total` DESC'''.format(cut_off_field,cut_off, s_string)

...
def hipsourcechains(database, df, filename):
...
#Create sheets
  ws = wb.active
  ws.title = 'Source Chain Total (Rev)'
  ws_1 = wb.create_sheet(title='Source Chain x Rev x {}'.format(period_name))
  ws_2 = wb.create_sheet(title='Source Chain Total (Lines)')
  ws_3 = wb.create_sheet(title='Source Chain x Lines x {}'.format(period_name))

#Sheet 1: Source Chain Total (Rev)
  #Find number of source chains
  find_source_chains = '''SELECT TRIM(Source_Chain_SB) FROM Master GROUP BY TRIM(Source_Chain_SB)'''
  mycursor.execute(find_source_chains)
  source_chains = [i[0] for i in mycursor.fetchall()]
  source_chain_count = len(source_chains)

  #Build table
  select_table_1 = '''SELECT TRIM(Source_Chain_SB), sum( CASE WHEN {} >= "{}" THEN Adjusted_Royalty_SB ELSE "" END) AS `Total` FROM Master 
    GROUP BY TRIM(Source_Chain_SB) ORDER BY `Total` desc '''.format(cut_off_field, cut_off)
  mycursor.execute(select_table_1)
  final_table_1 = mycursor.fetchall()

  #Create list of column names
  column_names_1 = ['Source Chain', 'Total']

  #Size of worksheet
  for column_no in range(1, len(column_names_1) + 1):
    for row_no in range(1, source_chain_count + 1):
  ...
  table_rows = ws[2:source_chain_count + 1]
  ...
  for song_no in range(2, source_chain_count + 2):
    ws.cell(row=song_no, column=1).alignment = Alignment(horizontal="left", vertical="center")
    ws.cell(row=song_no, column=2).number_format = '#,##0.00'

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) + 1), column_letters_2):
    ws.cell(row=source_chain_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_chain_count + 1)
    ws.cell(row=source_chain_count + 2, column=l).number_format = '#,##0.00'
    ws.cell(row=source_chain_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws.cell(row=source_chain_count + 2, column=l).font = Font(bold=True)
  ws.cell(row=source_chain_count + 2, column=2).number_format = '#,##0.00'
  ws.cell(row=source_chain_count + 2, column=1).value = 'Total'
  ws.cell(row=source_chain_count + 2, column=1).font = Font(bold='True')
  ws.cell(row=source_chain_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws.cell(row=source_chain_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

#Sheet 2: Source Chain x Rev x Half
  #Find number of source chains
  find_source_chains = '''SELECT TRIM(Source_Chain_SB) FROM Master GROUP BY TRIM(Source_Chain_SB)'''
  mycursor.execute(find_source_chains)
  source_chains = [i[0] for i in mycursor.fetchall()]
  ...
  #Build table
  select_table_1_1 = '''SELECT TRIM(Source_Chain_SB),'''
  select_table_1_2 = ""
  ...
  #Add table to worksheet
  table_rows = ws_1[2:source_chain_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      cell.value = j[k]
      cell.number_format = '#,##0.00'
  ...
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters_2):
    ws_1.cell(row=source_chain_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_chain_count + 1)
    ws_1.cell(row=source_chain_count + 2, column=l).number_format = '#,##0.00'
    ws_1.cell(row=source_chain_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_1.cell(row=source_chain_count + 2, column=l).font = Font(bold=True)
  ws_1.cell(row=source_chain_count + 2, column=1).value = 'Total'
  ws_1.cell(row=source_chain_count + 2, column=1).font = Font(bold='True')
  ws_1.cell(row=source_chain_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_1.cell(row=source_chain_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

  #Add % of revenue column
  total_column_letter = column_letters[len(column_names_1) - 3]
  for n in range(2, source_chain_count + 2):
    ws_1.cell(row=n, column=len(column_names_1) - 1).value = "=({}{}/{}{})".format(total_column_letter, n,
                                                                               total_column_letter, source_chain_count + 2)
  ...
  for o in range(3, source_chain_count + 2):
...
#Sheet 3: Source Chain Total (Lines)
  #Find number of source chains
  find_source_chains = '''SELECT TRIM(Source_Chain_SB) FROM Master GROUP BY TRIM(Source_Chain_SB)'''
  mycursor.execute(find_source_chains)
  source_chains = [i[0] for i in mycursor.fetchall()]
  ...
  #Build table
  select_table_1 = '''SELECT TRIM(Source_Chain_SB), COUNT( Source_Chain_SB) AS `Lines` FROM Master 
  WHERE {} >= "{}"
  GROUP BY TRIM(Source_Chain_SB) ORDER BY `Lines` desc '''.format(cut_off_field,cut_off)
  mycursor.execute(select_table_1)
  final_table_1 = mycursor.fetchall()

  #Create list of column names
  column_names_1 = ['Source Chain', 'Line Count']
  ...
  #Add table to worksheet
  table_rows = ws_2[2:source_chain_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      cell.value = j[k]
  ...
  for song_no in range(2, source_chain_count + 2):
    ...
    ws_2.cell(row=song_no, column=2).number_format = '#,##0'

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1)+1), column_letters_2):
    ws_2.cell(row=source_chain_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_chain_count + 1)
    ws_2.cell(row=source_chain_count + 2, column=l).number_format = '#,##0'
    ws_2.cell(row=source_chain_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_2.cell(row=source_chain_count + 2, column=l).font = Font(bold=True)
  ws_2.cell(row=source_chain_count + 2, column=2).number_format = '#,##0'
  ws_2.cell(row=source_chain_count + 2, column=1).value = 'Total'
  ws_2.cell(row=source_chain_count + 2, column=1).font = Font(bold='True')
  ws_2.cell(row=source_chain_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_2.cell(row=source_chain_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")

#Sheet 4: Source Chain x Lines x Half
  #Find number of source chains
  find_source_chains = '''SELECT TRIM(Source_Chain_SB) FROM Master GROUP BY TRIM(Source_Chain_SB)'''
  mycursor.execute(find_source_chains)
  source_chains = [i[0] for i in mycursor.fetchall()]
  ...
  for j, k in zip(year_statement_list, statement_list):
    select_table_1_2 += (
      ''' COUNT( IF(Year_Statement_9LC = "{}" AND {} = "{}",1,NULL)) `{} {}`,'''.format(j, mySQL_column, k, j, k))
  select_table_1_3 = '''COUNT( IF({} >= "{}",1,NULL)) `Total`
                     FROM Master
                     GROUP BY TRIM(Source_Chain_SB) ORDER BY `Total` DESC'''.format(cut_off_field, cut_off)
  select_table_1 = select_table_1_1 + select_table_1_2 + select_table_1_3
  mycursor.execute(select_table_1)
  ...
  for k in statement_period_list:
    column_names_1.append(k)
  column_names_1.append('Total')
  column_names_1.append('% Of Lines')
  column_names_1.append('Cumulative %')
  ...
  #Add table to worksheet
  table_rows = ws_3[2:source_chain_count + 1]
  for (row, j) in zip(table_rows, final_table_1):
    for (cell, k) in zip(row, range(len(j))):
      cell.value = j[k]
      cell.number_format = '#,##0'
  ...
  for song_no in range(2, source_chain_count + 2):
    ...
    ws_3.cell(row=song_no, column=(len(column_names_1) - 2)).font = Font(bold=True)

  #Add total row
  column_letters_2 = column_letters[1:]
  for (l, m) in zip(range(2, len(column_names_1) - 1), column_letters_2):
    ws_3.cell(row=source_chain_count + 2, column=l).value = "=SUM({}{}:{}{})".format(m, 2, m, source_chain_count + 1)
    ws_3.cell(row=source_chain_count + 2, column=l).number_format = '#,##0'
    ws_3.cell(row=source_chain_count + 2, column=l).alignment = Alignment(horizontal="right", vertical="center")
    ws_3.cell(row=source_chain_count + 2, column=l).font = Font(bold=True)
  ws_3.cell(row=source_chain_count + 2, column=1).value = 'Total'
  ws_3.cell(row=source_chain_count + 2, column=1).font = Font(bold='True')
  ws_3.cell(row=source_chain_count + 2, column=1).fill = PatternFill("solid", fgColor="A6ACAF")
  ws_3.cell(row=source_chain_count + 2, column=1).alignment = Alignment(horizontal="center", vertical="center")
